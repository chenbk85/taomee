...
static void send_to_sprite(void* key, void* value, void* data)
{
	...
	home_proto_t* pkg =(home_proto_t*) data;	
...
{
	hero_cup_game_t *game = (hero_cup_game_t*) obj;
...
static void end_game(hero_cup_game_t *game)
{
	...
	home_t *p_home;
	for(i = 0; i < 5; i++) {
		p_home	= (home_t*) g_hash_table_lookup(all_home, &pkg->homeid);
		if(p_home) {
...
static int game_loop(void* owner, void* data)
{
	...
	home_t *home = (home_t*)g_hash_table_lookup(all_home,&game->mapid);
	if(!home)  {
		return 0;
	}
...
{
	wait_group_t* g = NULL;
	g = (wait_group_t*) g_slice_alloc0(sizeof(wait_group_t));
...
static hero_cup_game_t* new_game_start(wait_group_t *g)
{
	...
	hero_cup_game_t* game =(hero_cup_game_t*) g->game;
	if(game == NULL) {
		game = (hero_cup_game_t* ) g_slice_alloc0(sizeof(hero_cup_game_t));
		next_dupid++;
		g->game = game;
		game->group = g;
...
int cancel_hero_cup_op(home_proto_t *pkg)
{
	...
	wait_group_t *g =(wait_group_t*) g_hash_table_lookup(all_player,&uid);
...
//	KDEBUG_LOG(pkg->id,"set_position %u",pos);
	...
	game = (hero_cup_game_t*) g_hash_table_lookup(all_games,&p_home->homeid);
...
int attack_boss_hero_cup_op(home_t* p_home, sprite_ol_t* p, home_proto_t *pkg)
{
	...
	if(game && LO32(p_home->homeid) == hero_cup_game_map) {
		...
		if(weapon->state == 0 && weapon->uid == p->id && weapon->lastatk + 2 < now) {
			...
			for(i = 0; i < 5; i++) {
				p_home	=(home_t*) g_hash_table_lookup(all_home, &mapid);
				if(p_home) {
					send_to_home(p_home,proto);
				}
				mapid++;
...
int get_positions_hero_cup_op(home_t* p_home, sprite_ol_t* p, home_proto_t *pkg)
{
	...
	game =(hero_cup_game_t*) g_hash_table_lookup(all_games,&mapid);
	if(!game) {
		cli->ret = htonl(err_invalid_home_map);
		cli->len = htonl(sizeof(client_proto_t));
