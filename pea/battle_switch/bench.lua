...
--so file
dll_file = "./libbtlsw.so"

-- 不同的配置
if (program_name == "btlswa") then
    require "./test"
else
    require "./dev"
end
...
local server_name = "pea.btlsw";
local bind_id = 8;
async_server_bind_map[0] = { bind_id, server_name, ip, 6100 + bind_id };
...
--pvp房间初始个数
init_pvp_room_num = 200;
--pve房间初始个数
init_pve_room_num = 200;
