...
#ifndef KFBTL_UTILS_HPP_
#define KFBTL_UTILS_HPP_
...
#define CHECK_VAL_LE(val_, max_) \
		do { \
			if ( (val_) > (max_) ) { \
				ERROR_TLOG("val %d is greater than max %d", (val_), (max_)); \
				return -1; \
			} \
		} while (0)
...
#define CHECK_VAL_GE(val_, min_) \
		do { \
			if ( (val_) < (min_) ) { \
				ERROR_TLOG("val %d is less than min %d", (val_), (min_)); \
				return -1;\
			} \
		} while (0)
...
#define CHECK_VAL_EQ(val_, expect_) \
		do { \
			if ( (val_) != (expect_) ) { \
				ERROR_TLOG("val %d is no equal to the expected %d", ((int)(val_)), ((int)(expect_))); \
				return -1; \
			} \
		} while (0)
