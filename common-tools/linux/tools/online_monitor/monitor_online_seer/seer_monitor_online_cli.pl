...
#my $seer_login_svr_addr = '10.1.1.4';
#my $seer_login_svr_port = '7788';

#wai wang 
#my $login_uid = $ARGV[0] || 88693; #jim's number
my $login_uid = $ARGV[0] || 37139509;
my $main_login_svr_addr = '114.80.99.73';
my $main_login_svr_port = '1863';
my $seer_login_svr_addr = '114.80.99.73';
my $seer_login_svr_port = '1863';

my $monitor_server_ip = '127.0.0.1';
my $monitor_sock; 
my $monitor_server_port = '7689';
...
open(ERROR_LOG_FILE, ">>/opt/taomee/monitor_online_seer/error_logfile") || die ("Could not open file");
open(DEBUG_LOG_FILE, ">>/opt/taomee/monitor_online_seer/debug_logfile") || die ("Could not open file");
...
if ( &init_seer_login_sock($seer_login_svr_addr, $seer_login_svr_port) != 0 ) {
	print ERROR_LOG_FILE "$now ERROR! INIT MAIN LOGIN SOCK\n";
	&send_data();
	goto exit_point;
}
if ( &get_seer_recommended_svr_list($login_uid) != 0 ) {
	print ERROR_LOG_FILE "$now ERROR! GET RECOMMENDED SVR LIST\n";
	&send_data();
	goto exit_point;
}
$data[1] = 1; #test seer recommended svr list pass
print DEBUG_LOG_FILE "$now GET RECOMMENDED SVR LIST SUCCESS\n";

#get the specific online, once get 10 online
if ( &get_1_online() != 0 ) {
...
exit_point:
	...
	if ( $seer_login_sock != 0 ) {
		close $seer_login_sock; $seer_login_sock = 0;
	}
close $monitor_sock;
exit 0;
...
{
	#send a request
    $sendbuf = pack("NcN3 NNN", 29, 1, 2101, $login_uid, $seqno++,
					100, 200, 0);
	...
	if ( &recv_respond_by_cmd($online_sock, 2101) != 0 ) {
		return -1;
	}
	return 0;
}
...
{
	#send a request
    $sendbuf = pack("NcN3 ", 17, 1, 2002, $login_uid, $seqno++);
    if ( send($online_sock, $sendbuf, 0) == 0 ) {
		diag("send error $!\n");
		return -1;
	}

	if ( &recv_respond_by_cmd($online_sock, 2002) != 0 ) {
		return -1;
	}

	#send a request
    $sendbuf = pack("NcN3 NNNN", 17 + 16, 1, 2001, $login_uid, $seqno++,
				0, 15, 100, 200);
    if ( send($online_sock, $sendbuf, 0) == 0 ) {
		diag("send error $!\n");
		return -1;
	...
	if ( &recv_respond_by_cmd($online_sock, 2001) != 0 ) {
		return -1;
	}
	return 0;
...
{
	#send a request
    $sendbuf = pack("NcN3 NNN", 29, 1, 2605, $login_uid, $seqno++,
					1, 100, 0);
    if ( send($online_sock, $sendbuf, 0) == 0 ) {
		diag("send error $!\n");
		return -1;
	...
	if ( &recv_respond_by_cmd($online_sock, 2605) != 0 ) {
		return -1;
	}
	return 0;
...
sub get_1_online 
{
	...
	$index = 1 if $index == 0;
	if ( &get_seer_ranged_svr_list($login_uid, $index, $index) != 0 ) {
		return -1;
	}
