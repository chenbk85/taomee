...
require Exporter;
our @ISA =qw(Exporter);
our @EXPORT =qw($main_login_sock $seer_login_sock $online_sock $recvbuf $length $cmdid $uid $result 
		init_main_login_sock login_main_loginSvr init_seer_login_sock get_seer_ranged_svr_list 
		get_seer_recommended_svr_list
		init_online_sock login_onlineSvr recv_respond_by_cmd);
our @version = 1.0;


our $main_login_sock = 0;
our $seer_login_sock = 0;
our $online_sock = 0;
our $length;
our $cmdid;
our $uid;
...
#functions related to seer login server
sub init_seer_login_sock;
sub get_seer_ranged_svr_list;
sub get_seer_recommended_svr_list;
...
sub init_seer_login_sock
{
	die("param error") if @_ != 2;
	(my $login_svr_addr, my $login_svr_port) = ($_[0], $_[1]);
	$seer_login_sock = IO::Socket::INET->new(
								PeerAddr => $login_svr_addr,
								PeerPort => $login_svr_port,
								Proto => 'tcp',
								Type => SOCK_STREAM);
...
sub login_onlineSvr
{
	...
	#login online server
#	$md5_str = "gOoD bYe crAzy BAby in Our ProgRAm?";
...
#	$digest = substr($digest, 6, 16);
	$sendbuf = pack("NcN3 a16", 33, 1, 1001, $login_uid, 0, 
					$sess_body) ;
	if ( send($online_sock, $sendbuf, 0) == 0 ) {
		return -1;
	}

	if ( &recv_respond_by_cmd($online_sock, 1001) != 0 ) {
		return -1;
	}
...
sub get_seer_recommended_svr_list
{
	...
	if ( send($seer_login_sock, $sendbuf, 0) == 0 ) {
		return -1;
	}

	if ( &recv_respond_by_cmd($seer_login_sock, 105) != 0 ) {
		return -1;
	}
...
sub get_seer_ranged_svr_list
{
	...
	if ( &recv_respond_by_cmd($seer_login_sock, 106) != 0 ) {
		return -1;
	}
