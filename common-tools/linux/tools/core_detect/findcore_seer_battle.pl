...
#work process!
		...
		print "here2\n";
		$exp->send("ls -lih BattleSvr*/core*\r");
        ...
        if($exp->match() eq "directory") {
			...
			$exp->send("mv -f BattleSvr*/core* ./core_bak/$now\r");
			$exp->expect(1,-re=>".*");
        }
...
if ( $core_file_cnt > 0 ) {
	#zog, evan, andy,crow
	`/home/kal/send_short_message/sendsms 13120781518,13916174663,13761071357,13621606625 "$core_file_cnt seer battle svr core! @msg_arr"`;
}
