...
int
CProxyDealMole::get_friends_list_cmd(request_info_t* info, uint8_t* body, int bodylen)
{
	int idx = sizeof(mole_db_proto_t);

	init_db_proto_head(pkgbuf, info, 0, mole_dbproto_get_friends_list, idx);
	return send_request_to_svr(info, pkgbuf, idx);
}

int
CProxyDealMole::get_user_nick_cmd(request_info_t* info, uint8_t* body, int bodylen)
{
	int idx = sizeof(mole_db_proto_t);

	init_db_proto_head(pkgbuf, info, 0, mole_dbproto_get_user_nick, idx);
	return send_request_to_svr(info, pkgbuf, idx);
}

int
CProxyDealMole::get_user_info_cmd(request_info_t* info, uint8_t* body, int bodylen)
{
	int idx = sizeof(mole_db_proto_t);

	init_db_proto_head(pkgbuf, info, 0, mole_dbproto_get_user_info, idx);
	return send_request_to_svr(info, pkgbuf, idx);
}

int
CProxyDealMole::get_user_cloth_info_cmd(request_info_t* info, uint8_t* body, int bodylen)
{
	int idx = sizeof(mole_db_proto_t);

	init_db_proto_head(pkgbuf, info, 0, mole_dbproto_get_user_cloth_info, idx);
	return send_request_to_svr(info, pkgbuf, idx);
}

int
CProxyDealMole::send_mail_cmd(request_info_t* info, uint8_t* body, int bodylen)
{
	send_mail_t* p_mail_info = reinterpret_cast<send_mail_t*>(body);

	if (p_mail_info->msg_len > sizeof(p_mail_info->msg)) {
		ERROR_LOG("gf email msg len error[%u %u]", info->userid, p_mail_info->msg_len);
	...
	pack_h(pkgbuf, mapid, idx);
	pack_h(pkgbuf, p_mail_info->msg_len, idx);
	pack(pkgbuf, p_mail_info->msg, p_mail_info->msg_len, idx);

	DEBUG_LOG("MOLE:SEND EMAIL\t[uid=%u mail=%u]", info->userid, p_mail_info->mail_type);
	init_db_proto_head(pkgbuf, info, 0, mole_dbproto_send_mail, idx);
	return send_request_to_svr(info, pkgbuf, idx);
}

int
CProxyDealMole::add_items_cmd(request_info_t* info, uint8_t* body, int bodylen)
{
	...
	pack_h(pkgbuf, 0, idx);
	pack_h(pkgbuf, 0, idx);
	DEBUG_LOG("MOLE:ADD ITEMS\t[uid=%u item_cnt=%u]",info->userid, p_item->item_cnt);
	for (uint32_t i = 0; i < p_item->item_cnt; i++) {
		...
		DEBUG_LOG("MOLE:ITEMS\t[type=%u id=%u cnt=%u max=%u]",p_item_info->item_type, 
				p_item_info->item_id, p_item_info->item_count, p_item_info->max_count);
	}
	
	init_db_proto_head(pkgbuf, info, 0, mole_dbproto_add_items, idx);
	return send_request_to_svr(info, pkgbuf, idx);
}

int
CProxyDealMole::handle_db_return(void* data, int len)
{
	mole_db_proto_t* pkg = reinterpret_cast<mole_db_proto_t*>(data);
	//int connfd = pkg->seq >> 16;
	//int idx = pkg->seq & 0xFFFF;

	//request_info_t* info = g_clients.get_request_info(connfd);
...
int 
CProxyDealMole::init_db_proto_head(void* header, request_info_t* info, uint32_t seq, uint16_t cmd, uint32_t len)
{
	mole_db_proto_t* proto = reinterpret_cast<mole_db_proto_t*>(header);
...
int
CProxyDealMole::dbproto_get_friends_list_callback(request_info_t* info, uint8_t* body, int body_len, uint32_t ret)
{
...
int
CProxyDealMole::dbproto_get_user_info_callback(request_info_t* info, uint8_t* body, int body_len, uint32_t ret)
{
	CHECK_DBERR(info, ret);
...
int
CProxyDealMole::dbproto_get_user_nick_callback(request_info_t* info, uint8_t* body, int body_len, uint32_t ret)
{
	CHECK_DBERR(info, ret);
...
int
CProxyDealMole::dbproto_send_mail_callback(request_info_t* info, uint8_t* body, int body_len, uint32_t ret)
{
	CHECK_DBERR(info, ret);
...
int
CProxyDealMole::dbproto_add_items_callback(request_info_t* info, uint8_t* body, int body_len, uint32_t ret)
{
	CHECK_DBERR(info, ret);
