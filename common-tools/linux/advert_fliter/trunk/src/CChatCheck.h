...
public:
	meta_msg_t* get_meta(message_t* msg);

	void update(uint64_t msgid, meta_msg_t* mmsg);
...
public:
	...
	CChatCheck(const char* word_file = NULL, const char* stat_file = NULL);
	~CChatCheck()
	{
		delete msg_meta_map;
		delete msg_user_map;
