...
int update_backup_svrlist(void* owner, void* data)
{
    ...
    //DEBUG_LOG("update backup svrlist");
    ADD_TIMER_EVENT(&get_backup_tmr, update_backup_svrlist, NULL, get_now_tv()->tv_sec + 60);
    return 0;
}
...
int login_mole_callback(usr_info_t* p, uint8_t* body, uint32_t bodylen, int ret)
{
    if ( ret) {
    ...
    mole_login_out* user_login_info = reinterpret_cast<mole_login_out*>(body);
    CHECK_VAL(bodylen, 8 + 4 * user_login_info->friendcount);
...
int get_recommended_svr_list(usr_info_t* p)
{
    ...
    taomee::pack(login_switch_sendbuf, (void*)(p->tmpinfo + 8), friendcount * sizeof(userid_t), idx);
...
int get_recommeded_svr_list_callback(usr_info_t* p, uint8_t* body, uint32_t bodylen, int ret)
{
    ...
    CHECK_VAL_GE(bodylen, sizeof(svr_list_t));
    svr_list_t* svrlist = reinterpret_cast<svr_list_t*>(body);
    CHECK_VAL(bodylen, 4 + sizeof(svr_list_t) + sizeof(svr_info_t) * svrlist->online_cnt);
...
int get_recommeded_svr_list_cmd(usr_info_t *p, uint8_t *body, uint32_t bodylen)
{
    ...
    if (p->uid != 0) {
        ...
        return send_request_to_db(p, DB_LOGIN_MOLE, NULL, 0);
    } else {
        return get_recommended_svr_list(p);
    }
}
...
/**
  ...
  * @return     int. 0 on success. less than 0 on error.
  */
int get_ranged_svr_list(usr_info_t* p, int start_id, int end_id, int friendcount, userid_t allfriendid[])
{
...
int get_ranged_svr_list_cmd(usr_info_t *p, uint8_t *body, uint32_t bodylen)
{
	...
	return get_ranged_svr_list(p, startid, endid, friendcount, allfriendid);
