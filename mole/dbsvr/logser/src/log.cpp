...
static int shift_fd (int lvl, time_t now)
{
	...
	tm = localtime (&now);
	if (likely ((size_t) length < log_size && fds_info[lvl].day == tm->tm_yday)) 
		return 0;
...
int log_init (const char* dir, int lvl, size_t size, const char* pre_name)
{
	int i, ret_code = -1;
