...
//-------------------PP特有的错误码 
//封号 
#define PP_NOUSED_ERR                       3320
//24小时离线
#define PP_OFFLIN24_ERR                     3321
//7天离线
#define PP_OFFLINE_SEVEN_DAY_ERR            3322
//14天离线
#define PP_OFFLINE_FOURTEEN_DAY_ERR         3323
...
//派派:检查用户是否存在
#define PP_CHECK_EXISTED_CMD                (0x180E )
//派派:加好友
#define PP_ADD_FRIEND_CMD                   (0x1830 |NEED_UPDATE)
//派派:大使：子节点的人数+1
#define PP_ADD_CHILD_COUNT_CMD              (0x183B |NEED_UPDATE)

typedef stru_id pp_add_friend_in;
...
struct  pp_login_out{
	uint32_t    vip_flag;
	uint32_t    Ol_today;
	stru_idlist friendlist;
	stru_idlist blacklist;
}__attribute__((packed));
...
class Cclientproto
{
  public:
    ...
    /* ====================  LIFECYCLE   ========================================= */
	//得到可用的userid
	int userid_get_userid( userid_get_userid_out *p_out );
	//设置该米米号仍然可用
	int userid_set_noused(userid_t userid );
	...
	//基础库登入
	int userinfo_login_ex( userid_t userid, char * passwd, uint32_t ip, 
	...
	int pp_login(userid_t userid,pp_login_out * p_out );

	//PP:检查是否存在 
	int pp_check_existed (userid_t userid,uint32_t * p_is_existed  );
	//PP:加好友
	int pp_add_friend (userid_t userid, userid_t friendid);
	//	派派:大使：子节点的人数+1 
	int pp_add_child_count (  userid_t userid );
...
//
	...
	int switch_pp_sync_vip(userid_t userid,uint32_t vipflag );
	char sendbuf[4096];
