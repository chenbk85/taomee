 ...
 * =====================================================================================
 *
 *       Filename:  Cuser_piglet_attire.cpp
 *
 *    Description:  
 *
 *        Version:  1.0
 *        Created:  09/06/2011 05:39:38 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
...
int Cuser_piglet_attire::add_attire_msglog(userid_t userid, uint32_t itemid, 
		uint32_t count, uint32_t type)
{
	...
	if(type == 1){//获取
		...
		else if(itemid >= 1593030 && itemid <= 1594000){
			msg_id = 0x0409BE01 + itemid - 1593030;
		
		}
		else if(itemid >= 1613000 &&  itemid <= 1614000){
			msg_id = 0x0409BEE0 + itemid - 1613000;
		}
		//else if(itemid >= 1613108 && itemid <= 1613113){
			//msg_id = 0x0409BD18 + itemid - 1613108;
		//}
		//else if(itemid >= 1613119 && itemid <= 1613125){
			//msg_id = 0x0409BD86;
		//}
		//else if(itemid >= 1613128 && itemid <= 1613133){
			//msg_id = 0x0409BDB6 + itemid - 1613128;
		//}
		//else if(itemid == 1613253){
			//msg_id = 0x0409BD20;
		//}
...
Cuser_piglet_attire::Cuser_piglet_attire(mysql_interface *db, Citem_change_log *p_log)
	:CtableRoute100x10(db, "USER", "t_user_piglet_attire", "userid")
{
...
int Cuser_piglet_attire::get_noused_count(userid_t userid, uint32_t itemid, uint32_t &count)
{
...
int Cuser_piglet_attire::get_count(userid_t userid, uint32_t itemid, uint32_t &count)
{
...
int Cuser_piglet_attire::add_count(userid_t userid, uint32_t itemid, uint32_t count, uint32_t limit,
...
int Cuser_piglet_attire::inner_add(userid_t userid, uint32_t itemid, uint32_t count, uint32_t limit)
{
	if(count == 0) 
	return SUCC;
	if(count > limit){
...
int  Cuser_piglet_attire:: get_items_range(userid_t userid, 
...
int Cuser_piglet_attire::remove(userid_t userid, uint32_t itemid)
{
...
int Cuser_piglet_attire::sub_count(userid_t userid, uint32_t itemid, uint32_t count, uint32_t is_vip_opt_type)
{
...
int Cuser_piglet_attire::take_off(userid_t userid, uint32_t itemid)
{
...
int Cuser_piglet_attire::put_on(userid_t userid, uint32_t itemid)
{
...
int Cuser_piglet_attire::inner_sub(userid_t userid, uint32_t itemid, uint32_t count)
{
	if(count == 0)
		return SUCC;
	uint32_t old_count = 0;
...
int Cuser_piglet_attire::get_interval_item(userid_t userid, uint32_t *p_in_item, uint32_t in_count, 
...
int Cuser_piglet_attire::check_items_enough(userid_t userid, uint32_t* material_items, uint32_t in_count,
