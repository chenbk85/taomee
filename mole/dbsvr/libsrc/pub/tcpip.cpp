...
/*
int
open_socket(const char *svrip, u_short svrport)
{
	int sockfd;
	...
	DEBUG_LOG("2val:0X%X",val );
	return sockfd;
}
*/
int
open_socket(const char *ipaddr, u_short port )
{
	int socketfd=safe_tcp_connect(ipaddr,port,2,0);	
	if (socketfd == -1){
		return socketfd;
	}
	//设置接收发送超时
