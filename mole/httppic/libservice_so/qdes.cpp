...
/** 
 * @brief  供外部模块使用，用key密钥加密s_test字符串，加密后的密文放在d_text中, 一次加密长度8字节。
 * @param  char *key, 密钥字符串
 * @param  char *s_text, 源明字符串，只有8字节会被加密。 
 * @param  char *d_text, 加密后的密文保存的缓冲区，一次只有8字节。 
 * @return void. 
 */
extern "C" void DES(char *key, char *s_text, char *d_text)
{
...
/** 
 * @brief  用供外部模块使用，key密钥解密s_test字符串，解密后的明文放在d_text中, 一次解密长度8字节。
 * @param  char *key, 密钥字符串
 * @param  char *s_text, 源密文字符串，只有8字节会被解密。 
 * @param  char *d_text, 解密后的明文保存的缓冲区，一次只有8字节。 
 * @return void. 
 */
extern "C" void _DES(char *key, char *s_text, char *d_text)
{
  des_key_schedule sch;
  des_set_key((des_cblock *)key, &sch);
 ...
 * @return  
 */
extern "C" int des_set_key(des_cblock *key, des_key_schedule* schedule)
{
	register DES_LONG c=0,d=0,t=0,s=0;
	register DES_LONG *k = NULL;

	register unsigned char *in = NULL;
	register int i=0;
 ...
 * @return  
 */
extern "C" void des_ecb_encrypt(des_cblock *input, des_cblock *output, des_key_schedule ks, int encrypt)
{
	register DES_LONG l;
 ...
 * @return  
 */
extern "C" void des_encrypt(DES_LONG *data, des_key_schedule ks, int encrypt)
{
...
/** 
 * @brief 供外部模块使用，加密（对DES的封装，去掉一次加密8个字节的限制)  
 * @param char* key, 加密的密钥 
 * @param int data_len, 被加密的数据字节数。  
 * @param void *src, 被加密的明文保存的缓存 
 * @param void *src, 被加密的密文保存的缓存, 调用者保证大小足够, >= data_len 
...
bool encrypt_data(char* key, int data_len, void *src, void *dest)
{
    ...
    int cblock_count = (data_len >> 3);
    int rest_len = data_len - (cblock_count << 3);
    int cblock_index = 0;
    while(cblock_index < cblock_count)
    {
        DES(key, psrc, pdest);
        cblock_index++;
        psrc += 8,
    ...
    if(rest_len > 0)
    {
        //char tmp_src[8]={0};
        //char tmp_dest[8]={0};
        //memcpy(tmp_src, psrc, rest_len);
        //DES(key, tmp_src, tmp_dest);
        //memcpy(pdest, tmp_dest, rest_len);
        DES(key, psrc, pdest);
    }
    return true;
}
...
/** 
 * @brief 供外部模块使用，解密（对_DES的封装，去掉一次加密8个字节的限制)  
 * @param char* key, 解密的密钥 
 * @param int data_len, 被解密的数据字节数。  
 * @param void *src, 被解密的密文保存的缓存 
 * @param void *src, 被解密的明文保存的缓存, 调用者保证大小足够, >= data_len 
 * @return 成功返回true, 失败返回false.  
 */
