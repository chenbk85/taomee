...
int do_add_conn(int fd, uint8_t type, struct sockaddr_in *peer, bind_config_elem_t* bc_elem)
{
	...
	if ((type == PIPE_TYPE_FD) || (type == MCAST_TYPE_FD))
		flag = EPOLLIN;
	else
...
void do_del_conn(int fd, int is_conn)
{
	...
	if (is_conn == 0) {
		handle_close(fd, 0, 1);
	} else if ((is_conn == 1) && epi.fds[fd].bc_elem) {
		struct shm_block mb;
		mb.id     = epi.fds[fd].id;
		mb.fd     = fd;
		mb.type   = CLOSE_BLOCK;
...
static int do_open_conn(int fd, int isconn)
{
	...
	newfd = mole_safe_tcp_accept(fd, &peer);
	if (newfd > 0) {
		epi.fds[newfd].sk.last_tm = get_now_tv()->tv_sec;
		do_add_conn(newfd, REMOTE_TYPE_FD, &peer, 0);
/*
		do_add_conn(newfd, REMOTE_TYPE_FD, &peer, epi.fds[fd].bc_elem);
		...
		if (isconn) {
			...
			if (shmq_push(&(epi.fds[newfd].bc_elem->recvq), &mb, (void*)&epi.fds[newfd].sk) == -1) {
				do_del_conn(newfd, 2);
			}
		}*/
	}
...
int do_write_conn (int fd)
{
	...
	send_bytes = mole_safe_tcp_send_n (fd, epi.fds[fd].cb.sendptr, epi.fds[fd].cb.sendlen);
	if (send_bytes == 0)
		return 0;
...
static int do_read_conn(int fd, int max)
{
	...
	if (!epi.fds[fd].cb.recvptr) {
		...
		epi.fds[fd].cb.recvptr    = mmap(0, PAGESIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
		if (epi.fds[fd].cb.recvptr == MAP_FAILED) {
			ERROR_RETURN(("mmap failed"), -1);
		}
	}

	if (PAGESIZE == epi.fds[fd].cb.recvlen) {
		TRACE_LOG("recv buffer is full, fd=%d", fd);
		return 0;
	}

	recv_bytes = mole_safe_tcp_recv(fd, epi.fds[fd].cb.recvptr + epi.fds[fd].cb.recvlen,
...
//------------------------------------
			protocol_t* proto = (void*)epi.fds[fd].cb.recvptr;
			bind_config_t* bc = get_bind_conf();

			if (((ntohl(proto->cmd) == PROTO_LOGIN) || (ntohl(proto->cmd) == PROTO_VERSION)) && (epi.fds[fd].bc_elem == 0) ) {
				if(ntohl(proto->cmd) == PROTO_VERSION) {
					epi.fds[fd].bc_elem = &(bc->configs[0]);
				} else {
					if (epi.fds[fd].cb.rcvprotlen < (sizeof(*proto) + 2)) {
						epi.fds[fd].cb.recvptr = saved_recvptr;
						do_del_conn(fd, 2);
						ERROR_RETURN(("invalid proto len %d", epi.fds[fd].cb.rcvprotlen), 0);
					}

					uint16_t online_id = ntohs(*(uint16_t*)(proto->body));
					if ((online_id < bc->online_start_id) || (online_id >= (bc->online_start_id + bc->bind_num))) {
						epi.fds[fd].cb.recvptr = saved_recvptr;
						do_del_conn(fd, 2);
						ERROR_RETURN(("invalid online id %d", online_id), 0);
					}
					epi.fds[fd].bc_elem = &(bc->configs[online_id - bc->online_start_id]);
				}
				...
				mb.type   = OPEN_BLOCK;
				mb.length = sizeof(mb) + sizeof(struct skinfo);
				if (shmq_push(&(epi.fds[fd].bc_elem->recvq), &mb, (void*)&epi.fds[fd].sk) == -1) {
					epi.fds[fd].cb.recvptr = saved_recvptr;
					do_del_conn(fd, 2);
					return 0;
				}
			...
			if (epi.fds[fd].bc_elem == 0) {
				epi.fds[fd].cb.recvptr = saved_recvptr;
				do_del_conn(fd, 2);
				ERROR_RETURN(("fd %d not logined yet", fd), 0);
			}
//------------------------------------
...
int net_start(const char* listen_ip, in_port_t listen_port, bind_config_elem_t* bc_elem)
{
	...
	listenfd = mole_safe_socket_listen (&servaddr, SOCK_STREAM);
	...
	if (listenfd > 0) {
		...
		do_add_conn(listenfd, LISTEN_TYPE_FD, 0, bc_elem);
		ret_code = 0;
	}
...
/**
 * connect_to_svr - connect to server @ipaddr:@port
 *
 * return: connected fd
 * caution: !!only for the child process!!
 */
int connect_to_svr (const char *ipaddr, uint16_t port, int bufsz, int timeout)
{
	...
	if (inet_pton (AF_INET, ipaddr, &peer.sin_addr) <= 0)
		ERROR_RETURN (("inet_pton %s:%u failed, %m", ipaddr, port), -1);

	fd = mole_safe_tcp_connect (ipaddr, port, timeout);
	if (fd != -1) {
		DEBUG_LOG("connect to %s:%u", ipaddr, port);
		do_add_conn(fd, REMOTE_TYPE_FD, &peer, 0);
...
int net_send(int fd, const uint8_t *data, uint32_t len, int is_conn)
{
	...
	send_bytes = 0;
	if (epi.fds[fd].cb.sendlen == 0) {
		send_bytes = mole_safe_tcp_send_n(fd, data, len);
		if (send_bytes == -1) {
			do_del_conn(fd, is_conn);
			//if (is_conn) {
			//	do_del_conn(fd, (epi.fds[fd].bc_elem ? is_conn : 2));
			//} else {
			//	do_del_conn(fd, is_conn);
			//}
			return -1;
...
#ifdef USE_CMD_QUEUE
//iterate over the busy sprite list and  handle each sprite's pending cmd queue
void handle_cmd_busy_sprite()
{
	...
	list_for_each_safe(cur, next, &g_busy_sprite_list) {
		sprite_t *p = list_entry(cur, sprite_t, busy_sprite_list_hook);
		assert( p != NULL );
		//handle the cmd queue until blocked by slow db request
		while ( p->waitcmd == 0 ) {
			//at least has a pending cmd, so no need to check emptiness
			gpointer cmdbuf = g_queue_pop_head(p->pending_cmd_queue);
			assert(cmdbuf != NULL);
			uint32_t cmdlen = *(uint32_t*)cmdbuf - 4; // cmd length
			DEBUG_LOG("BEGIN HANDLE ONE PENDING CMD\t[uid=%u, cmdid=%u, cmdlen=%u]",
						p->id, p->waitcmd, cmdlen);
			p->pend_flag = 1;
			int err = parse_protocol((uint8_t*)cmdbuf + 4, cmdlen, p->fdsess);
			g_slice_free1(cmdlen + 4, cmdbuf);
			if (!err) {
				...
				shm_ctl_block_push(&(config_cache.bc_elem->sendq), p->fd, FIN_BLOCK, 1);
				break;
			}
		}
	}
...
int net_loop(int timeout, int max_len, int is_conn)
{
	...
	for (pos = 0; pos < nr; pos++) {
		...
		if ( unlikely(epi.fds[fd].type == PIPE_TYPE_FD) ) {
			if (epi.evs[pos].events & EPOLLHUP) {
				if (is_conn) { // Child Crashed
					...
					CRIT_LOG("CHILD PROCESS CRASHED!\t[olid=%d olname=%s]", epi.fds[pfd].bc_elem->online_id, epi.fds[pfd].bc_elem->online_name);
					restart_child_process(epi.fds[pfd].bc_elem);
				} else { // Parent Crashed
					stop = 1;
					return -1;
				}
			} else {
				while ( read(fd, trash, PAGESIZE) == PAGESIZE ) ;
			}
			continue;
		}
		...
		if (epi.evs[pos].events & EPOLLIN) {
			...
			} else if (epi.fds[fd].type == MCAST_TYPE_FD) {
				static char buf[PAGESIZE];
				int  i;
				for (i = 0; i != 100; ++i) {
					int len = recv(fd, buf, PAGESIZE, MSG_DONTWAIT);
					if (len > 0) {
						handle_mcast_pkg((void*)buf, len);
					} else {
						break;
					}
				}
