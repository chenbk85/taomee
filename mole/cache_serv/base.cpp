...
int is_user_exist(uint32_t gm_id, uint32_t user_id, uint32_t score, int32_t *opy_pos)
{
	int j;
	int index;
	gmsc_ptr_t p;
	...
	if (gm_id < 0 || gm_id > max_game) {
		ERROR_LOG("is_user_exit : the game id is wrong");
		return 0;
	}

	index = gm_id - 1;
	/*get game score address*/
	p = (gmsc_adr + index)->game_score;
	/*judge the user is exist or not*/
	for (j = 0; j < GMSC_FIRST_NUM && p[j].user_id != user_id; j++) {
		;
    }
	if (j >= GMSC_FIRST_NUM) {
		return 0;
	}
	/*judge the old score bigger than the new or not*/
	*opy_pos = (p[j].score >= score) ? -1 : j;
	return 1;
}
