...
int
safe_tcp_recv (int sockfd, void *buf, int bufsize)
{
	int cur_len;
recv_again:
...
int
safe_tcp_recv_n (int sockfd, void *buf, int total)
{
 ...
 */
int
safe_tcp_send_n (int sockfd, const void *buf, int total)
{
	...
	for (send_bytes = 0; send_bytes < total; send_bytes += cur_len)
	{
		...
		else if (cur_len == -1)
		{
			if (errno == EINTR)
				cur_len = 0;
			// EWOULDBLOCK == EAGAIN under Linux
			else if (errno == EAGAIN)
			{
				return send_bytes;
			}
 ...
 */
int
safe_socket_listen (struct sockaddr_in *servaddr, int type)
{
	int listenfd;
	int reuse_addr = 1;
 ...
 */
int
safe_tcp_accept (int sockfd, struct sockaddr_in *peer)
{
	socklen_t peer_size;
	int newfd, val, err;
	...
	for (;;)
	{
		...
		if ((newfd = accept (sockfd, (struct sockaddr *) peer, &peer_size)) < 0)
		{
			...
			ERROR_RETURN(("accept failed: listenfd=%d err=%d %s", sockfd, errno, strerror(errno)), -1);
		}
...
int
safe_tcp_connect (const char *ipaddr, u_short port, int timeout)
{
