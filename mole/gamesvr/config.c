...
static int
str2int (const char *strval, int def)
{
	int ret_code = def;
...
static int
config_put_value (const char *key, const char *val)
{
...
char *
config_get_strval (const char *key)
{
	list_head_t * hlist;
	struct config_pair *mc;
...
int
config_get_intval (const char *key, int def)
{
...
int
mmap_config_file (const char *file_name, char **buf)
{
	...
	*buf =
		mmap (0, len + 1, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1,
				0);
	if (*buf == MAP_FAILED)
	{
...
int
config_init (const char *file_name)
{
