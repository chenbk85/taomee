...
private:
    ...
    int m_id_;                                             // 下一次连接可分配的ID
    int m_id_base_;                                        // 连接ID生成基质
    int m_id_step_;                                        // 连接ID增长步长
    int m_errno_;                                          // 上一次出错的错误码 
    char m_errstr_[MAX_ERRSTR_LEN];                        // 上一次出错的错误信息
    int m_listen_fd;                                       // 监听地址对应的文件描述符
    int m_epoll_fd;                                        // EPOLL对应的文件描述符 
    id_conn_map_t m_id_conn_map_;                          // 连接ID和连接信息的MAP
    i_net_io_event_handler* m_p_net_io_event_handler;      // 网络IO事件处理对象
    i_net_io_notifier *m_p_net_io_notifier_;               // 通知机制对象
    int m_flag_;                                           // 全局状态标志(现在包括文件描述符使用完的状态标志)
