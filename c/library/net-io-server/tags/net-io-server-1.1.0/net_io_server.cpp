...
int c_net_io_server::get_connection_info(const int connection_id, char *server_ip, int ip_buffer_len,
                                  int *p_port, int *p_connected_time)
{
    if(!m_inited)
    {
        return -1;
...
int c_net_io_server::close_connection(const int connection_id, bool b_notify)
{
    ...
    else
    {
        if(b_notify)
        {
            ...
            if(m_p_events)
            {
                m_p_events->on_connection_closed((void*)dynamic_cast<c_net_io_server*>(this), iter->first);
            }
        }
                        ...
                        if(peer_socket >= 0)
                        {
                            ...
                            if(m_p_events)
                            {
                                if(-1 == m_p_events->on_new_connection((void*)dynamic_cast<c_net_io_server*>(this),
                                                conn_info.connection_id, conn_info.peer_ip, conn_info.peer_port))
                                {
                                    close(peer_socket);
                                    continue;
                                }
			                ...
			                else
			                {
                                ...
                                if(m_p_events)
                                {
                                    m_p_events->on_connection_closed((void*)dynamic_cast<c_net_io_server*>(this), conn_info.connection_id);
                                }
                                close(peer_socket);
                                continue;
			                }
                    ...
                    if(events[index].events & EPOLLIN)
                    {
                        while(true)
                        {
                            ...
                            if(bytes_recv > 0)
                            {
                                if(m_p_events)
                                {
                                    if(m_p_events->on_recv_data((void*)dynamic_cast<c_net_io_server*>(this), p_conn->connection_id, recv_buffer, bytes_recv) < 0)
                                    {
