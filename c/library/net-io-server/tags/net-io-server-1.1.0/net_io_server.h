...
public:
    c_net_io_server();
    ~c_net_io_server();
    int init(const char *listen_ip, const int listen_port, const i_net_io_server_events *p_events);
    ...
    int get_connection_info(const int connection_id, char *server_ip, int ip_buffer_len, int *p_port, int *p_connected_time);
...
protected:
    ...
    typedef struct
    {
        ...
        int buffer_data_len;		/**< 发送缓存中数据的长度*/
        char send_buffer[MAX_BUFFER_LEN];/**< 发送缓存*/
    }connection_info_t;
