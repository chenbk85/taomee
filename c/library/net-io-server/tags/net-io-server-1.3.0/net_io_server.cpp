...
int c_net_io_server::send_data(const int connection_id, char *p_data, const int data_len)
{
    ...
    int empty_buffer_len = sizeof(iter->second.send_buffer) - iter->second.send_buffer_data_len;
    ...
    memcpy(iter->second.send_buffer + iter->second.send_buffer_data_len, p_data + bytes_sent, copy_len);
    iter->second.send_buffer_data_len += copy_len;
...
int c_net_io_server::send_data_atomic(const int connection_id, char *p_data, const int data_len)
{
    ...
    memcpy(iter->second.send_buffer + iter->second.send_buffer_data_len, p_data + bytes_sent, remain_data_len);
    iter->second.send_buffer_data_len += remain_data_len;
    return data_len;
}
...
//                            }
                ...
                if(events[index].data.ptr != NULL)
                {
                    ...
                    if(events[index].events & EPOLLIN)
                    {
                        while(true)
                        {
                            ...
                            int empty_buffer_len = sizeof(p_conn->recv_buffer) - p_conn->recv_buffer_data_len;
                            int bytes_recv = recv(p_conn->sock_fd, p_conn->recv_buffer + p_conn->recv_buffer_data_len, empty_buffer_len, 0);
                            if(bytes_recv > 0)
                            {
                                p_conn->recv_buffer_data_len += bytes_recv;

                                if(m_p_events)
                                {
                                    int bytes_left = m_p_events->on_recv_data((void*)dynamic_cast<c_net_io_server*>(this), p_conn->connection_id, p_conn->sock_fd, p_conn->recv_buffer, p_conn->recv_buffer_data_len);

                                    if(bytes_left < 0 || bytes_left >= (int)sizeof(p_conn->recv_buffer))
                                    {
                                        //invalid return value,close the connection
                                    ...
                                    {
                                        p_conn->recv_buffer_data_len = bytes_left;
                                    }
                                }
                                else
                                {
                                    //do not define recv events,discard all data in slience
                                    p_conn->recv_buffer_data_len = 0;
                                }

                                if(bytes_recv < empty_buffer_len)
                                {
                                    //没有更多的数据可接受
                                    break;
                                }
    ...
    if(flags & NET_IO_SERVER_OP_WRITE)
    {
        ...
        {
            if(id_iter->second.send_buffer_data_len > 0)
            {
                int bytes_sent = send(id_iter->second.sock_fd, id_iter->second.send_buffer, id_iter->second.send_buffer_data_len, 0);
                if(bytes_sent > 0)
                {
                    memmove(id_iter->second.send_buffer, id_iter->second.send_buffer + bytes_sent,
                            id_iter->second.send_buffer_data_len - bytes_sent);
                    id_iter->second.send_buffer_data_len -= bytes_sent;
                }
                else
                {
