...
int c_net_io_server::do_io(int timeout, int cmd)
{
    if (!m_init_)
    {
        return -1;
    ...
    for (int i = 0; i < epoll_ret; ++ i)
    {
        ...
        if (m_events_[i].data.ptr)
        {
                ...
                && p_connection_info->buffer_send_len > 0)
            {
                ...
                if (send_data_atomic_(p_connection_info, NULL, 0) < 0)
                {
...
int c_net_io_server::send_data_(connection_info_t *p_connection_info)
{
    int send_len = 0;
    ...
    int extra_buffer_offset = 0;

    if (left > 0)
    {
        if (left < p_connection_info->buffer_send_len)
        {
    ...
    {
        if (p_connection_info->extra_buffer_len > 0)
        {
            // 这里表明当前发送缓冲区已空,继续发送extra缓冲区数据
            p_connection_info->buffer_send_len = 0;
            left = p_connection_info->extra_buffer_len_used;

            while (left > 0)
            {
                send_len = send(p_connection_info->fd,
                                p_connection_info->extra_buffer
                                + p_connection_info->extra_buffer_len_used
                                - left,
                                left,
                                0);
            ...
            extra_buffer_offset
                = p_connection_info->extra_buffer_len_used - left;
        }
        else
        {
            p_connection_info->buffer_send_len = 0;
    ...
    if (p_connection_info->extra_buffer_len_used > 0)
    {
        // 将extra缓冲区数据copy部分到发送缓冲区
        int buffer_empty_len
            = MAX_BUFFER_SEND_LEN - p_connection_info->buffer_send_len;
        int extra_buffer_len_real_used
            = p_connection_info->extra_buffer_len_used - extra_buffer_offset;

        if (extra_buffer_len_real_used > 0)
        {
            if (extra_buffer_len_real_used <= buffer_empty_len)
            {
                // 可以将extra部分完全考如发送缓冲区中
                memcpy(p_connection_info->buffer_send
                       + p_connection_info->buffer_send_len,
                       p_connection_info->extra_buffer + extra_buffer_offset,
                       extra_buffer_len_real_used);
                p_connection_info->buffer_send_len
                    += extra_buffer_len_real_used;
                left = p_connection_info->buffer_send_len;
                p_connection_info->extra_buffer_len_used = 0;
            }
            else
            {
                memcpy(p_connection_info->buffer_send
                       ...
                       p_connection_info->extra_buffer + extra_buffer_offset,
                       buffer_empty_len);
                p_connection_info->buffer_send_len = MAX_BUFFER_SEND_LEN;
                memmove(p_connection_info->extra_buffer,
                        p_connection_info->extra_buffer
                        + extra_buffer_offset
                        + buffer_empty_len,
                        p_connection_info->extra_buffer_len_used
                        = (extra_buffer_len_real_used - buffer_empty_len));
                left = p_connection_info->extra_buffer_len_used
                       + MAX_BUFFER_SEND_LEN;
            }
        }
...
{
    int send_len = 0;
    int left = send_data_(p_connection_info);
    ...
    if (left > 0)
    {
        ...
        if (p_connection_info->buffer_send_len < MAX_BUFFER_SEND_LEN)
        {
            int buffer_empty_len
                = MAX_BUFFER_SEND_LEN - p_connection_info->buffer_send_len;
        ...
        {
            int buffer_empty_len
                = p_connection_info->extra_buffer_len
                  - p_connection_info->extra_buffer_len_used;
            ...
            if (buffer_empty_len > 0)
            {
                if (left <= buffer_empty_len)
                {
                    memcpy(p_connection_info->extra_buffer
                           + p_connection_info->extra_buffer_len_used,
                           p_data + data_len - left,
                           left);
                    p_connection_info->extra_buffer_len_used += left;
    ...
    if (0 == p_connection_info->extra_buffer_len_used
        && p_connection_info->extra_buffer_len > 0)
    {
        free_(p_connection_info->extra_buffer);
        p_connection_info->extra_buffer = NULL;
        p_connection_info->extra_buffer_len_used
            = p_connection_info->extra_buffer_len
            = 0;
    }
...
int c_net_io_server::send_data(int id, const char *p_data, int data_len)
{
    if (!m_init_)
    {
        return -1;
    ...
    return send_data_(p_connection_info, p_data, data_len);
}

int c_net_io_server::send_data_atomic(int id, const char *p_data, int data_len)
{
    if (!m_init_)
    {
        return -1;
    ...
    connection_info_t *p_connection_info
            = reinterpret_cast<connection_info_t *>(&it->second);
    return send_data_atomic_(p_connection_info, p_data, data_len);
}

int c_net_io_server::send_data_atomic_(connection_info_t *p_connection_info,
                                       const char *p_data,
                                       int data_len)
{
    int send_len = 0;
    ...
    if ((data_len + left) > (MAX_BUFFER_EXTRA_LEN + MAX_BUFFER_SEND_LEN))
    {
        // 若输出缓冲区的最大容量不够放入新数据,则不进行新书据发送或拷贝,
        // 以保证发送操作原子性
        return 0;
    }
    ...
    if (left > 0)
    {
        int buffer_len_need = left
                              + p_connection_info->buffer_send_len
                              + p_connection_info->extra_buffer_len_used;
        int extra_buffer_len_need = buffer_len_need - MAX_BUFFER_SEND_LEN;

        if (extra_buffer_len_need > p_connection_info->extra_buffer_len)
        {
            int extra_buffer_len_alloc
                = (extra_buffer_len_need + getpagesize() - 1)
                  / getpagesize() * getpagesize();
            char *extra_buffer = NULL;

            if (p_connection_info->extra_buffer_len > 0)
            {
                extra_buffer = reinterpret_cast<char *>(
                                realloc_(p_connection_info->extra_buffer,
                                         extra_buffer_len_alloc));
            }
            else
            {
                extra_buffer = reinterpret_cast<char *>(
                                malloc_(extra_buffer_len_alloc));
            }
            ...
            p_connection_info->extra_buffer_len = extra_buffer_len_alloc;
            p_connection_info->extra_buffer = extra_buffer;
        }

        int delta = MAX_BUFFER_SEND_LEN - p_connection_info->buffer_send_len;
        ...
        else
        {
            if (delta > 0)
            {
                ...
                memcpy(p_connection_info->extra_buffer,
                       p_data + data_len - left + delta,
                       left - delta);
                p_connection_info->extra_buffer_len_used += (left - delta);
            }
            else
            {
                // 发送缓冲区完全不够
...
int c_net_io_server::broadcast(int id,
                               const char *p_data,
                               int data_len,
                               int is_atomic)
{
...
int c_net_io_server::close_connection(int id, bool need_notify)
{
    if (!m_init_)
    {
        return -1;
    ...
    if (it->second.extra_buffer_len > 0)
    {
        free_(it->second.extra_buffer);

        //it->second.extra_buffer_len_used
            //= it->second.extra_buffer_len
            //= 0;
    }
...
int c_net_io_server::get_connection_info(int id,
                                         char *ip,
                                         int ip_buffer_len,
                                         int *p_port,
                                         int *p_time,
