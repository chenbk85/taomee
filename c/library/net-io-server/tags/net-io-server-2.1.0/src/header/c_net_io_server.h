...
#define MAX_BUFFER_EXTRA_LEN  (2 * 1024 * 1024)
...
public:
    ...
    typedef struct
    {
        ...
        int buffer_send_len;
        int extra_buffer_len;
        int extra_buffer_len_used;
        char *extra_buffer;
     ...
     */
    int send_data_atomic(int id, const char *p_data, int data_len);
     ...
     * @return 0-成功 -1-失败
     */
    int get_connection_info(int id,
                            char *ip,
                            int ip_buffer_len,
                            int *p_port,
                            int *p_time,
...
private:
     ...
     * @brief 发送对应对客户端的残留数据
     *
     * @param p_connection_info  指向连接对象结构
     *
     * @return >=0-缓冲区中剩余的数据大小 -1-失败(此时连接也被关闭)
     */
    int send_data_(connection_info_t *p_connection_info);

    /**
     * @brief 发送数据,行为同send_data,仅参数不同,对象作为内部调用
     *
     * @param p_connection_info  指向连接对象结构
     * @param p_data 欲发送的数据缓存
     * @param data_len  发送数据长度
     *
     * @return  >=0-处理数据的总长度 -1-失败(此时连接也被关闭)
     */
    int send_data_(connection_info_t *p_connection_info,
                   const char *p_data,
                   int data_len);
     ...
     * @param data_len  发送数据长度
     *
     * @return data_len-成功 -1-失败(此时连接也被关闭)
     */
    int send_data_atomic_(connection_info_t *p_connection_info,
                          const char *p_data,
                          int data_len);
};
