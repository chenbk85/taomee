...
int c_ini_file::parse_file(const char *section, const char *key, int *sec_s, int *sec_e, int *key_s,int *key_e, int *value_s, int *value_e)
{
    ...
    *sec_e = *sec_s = *key_e = *key_s = *value_s = *value_e = -1;
    while(!end_of_string(p[i])) {
        //find the section
        if(((i == 0) || newline(p[i-1])) && left_barce(p[i])) {
            ...
            if((strlen(section)==(size_t)(i-section_start)) && (strncmp(p+section_start,section, i-section_start) == 0)) {
                int newline_start=0;
                ...
                while(! (newline(p[i-1]) && left_barce(p[i])) && !end_of_string(p[i])) {
                    ...
                    if(p[j] != ':') {  //skip over comment
                        while(j < i && p[j]!='=') {
                            j++;
                            if('=' == p[j]) {
                                if((strlen(key)==(size_t)(j-newline_start)) && (strncmp(key,p+newline_start,j-newline_start)==0)) {
                                    //find the key ok
                                    *key_s = newline_start;
                                    *key_e = j-1;
...
int c_ini_file::write_string(const char *section, const char *key, const char *value)
{
    ...
    if (m_file_size == 0) {
        ...
        if (sec_s == -1) {
            ...
            /// 更新值
            int idx = 0;
            int append_len = value_len - value_e + value_s;
            if (m_file_size + append_len > MAX_FILE_BUF_SIZE) {
                set_err(INI_FILE_E_BUF_OVERFLOW);
                return -1;
            }
            ...
            if (append_len <= 0) {
                /// 向前移动
                for (idx = value_e + append_len; idx <= m_file_size + append_len; idx++) {
                    m_file_buf[idx] = m_file_buf[idx - append_len];
                }
            }
            else {
                /// 向后移动
                for (idx = m_file_size + append_len; idx >= value_e + append_len; idx--) {
                    m_file_buf[idx] = m_file_buf[idx - append_len];
                }
            }
