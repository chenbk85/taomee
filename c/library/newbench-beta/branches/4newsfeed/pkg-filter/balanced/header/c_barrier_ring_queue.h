...
loop:
    ...
    if (!p_assist_array_node->flag)
    {
        // 无须进行屏障功能,则直接摘走used list节点
        m_last_flag = 0;
        used_list_del(p_assist_array_node);
        push_empty_list_node(p_assist_array_node);
        *pp_assist_array_node = p_assist_array_node;
    ...
    m_last_flag = p_assist_array_node->flag;    // 大于零的进程ID标示冲突了!
    m_last_key = p_assist_array_node->key;
    *pp_assist_array_node = m_p_assist_array_node = p_assist_array_node;
    m_p_assist_array_node->flag = static_cast<int32_t>(getpid());
...
inline int c_barrier_ring_queue::is_able_pop()
{
    ...
    return m_p_meta->used_list.p_read_head || (m_last_flag && !list_empty(&m_p_assist_array_node->homo_list));
}
