...
int c_mysql_iface::select_first_row(MYSQL_ROW* row, const char* sql_fmt, ...)
{
    ...
    m_hdb = get_conn();
    if (m_hdb == NULL) {
        set_err(MYSQL_IFACE_E_NOT_INITED, "Db uninited while select one row.");
        return -1;
    }
    ...
    if (mysql_query(m_hdb, m_sql_buf) != 0) {
        set_err(MYSQL_IFACE_E_MYSQL_API, "Mysql query error: %s\n\tSQL: %s", mysql_error(m_hdb), m_sql_buf);
        return -1;
    }
    ...
    m_res = mysql_store_result(m_hdb);
    if (m_res == NULL) {
...
int c_mysql_iface::execsql(const char* sql_fmt, ...)
{
    ...
    return mysql_affected_rows(m_hdb);
}
