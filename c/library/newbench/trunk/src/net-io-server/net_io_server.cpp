...
net_io_server::net_io_server() : m_inited(false), m_id(0), m_errno(NET_IO_SERVER_OK),
                                 m_p_net_io_notifier(NULL), m_flag(0), 
                                 m_p_cont_mem(NULL), m_cont_mem_len(0)
{
    bzero(m_errstr, sizeof(m_errstr));
    m_id_conn_map.clear();
...
{
    TRACE_LOG("net_io_server::init");
    if (m_inited) {
        TRACE_LOG("ERROR: m_inited");
        return -1;
    }

    if (!ip || port < 0 || port > 65535) {
        TRACE_LOG("ERROR: !ip || port < 0 || port > 65535");
        return -1;
    }

    if (id_base <= 0 || id_step <= 0) {
        TRACE_LOG("ERROR: id_base <= 0 || id_step <= 0");
        return -1;
    }
    ...
    if (m_listen_fd < 0) {
        set_error_info(NET_IO_SERVER_EINIT, "fail to obtain socket(%s)", strerror(errno));
        return -1;
    }
    ...
    if (sockaddr_listen.sin_addr.s_addr == INADDR_NONE) {
        close(m_listen_fd);
        set_error_info(NET_IO_SERVER_EINIT, "fail to inet_addr(%s)", strerror(errno));
        return -1;
    }
    ...
    if (setsockopt(m_listen_fd, SOL_SOCKET, SO_REUSEADDR, &is_reuseaddr, sizeof(int)) != 0) {
        close(m_listen_fd);
        set_error_info(NET_IO_SERVER_EINIT, "fail to setsockopt(%s)", strerror(errno));
        return -1;
    }
    ...
    if (bind(m_listen_fd, reinterpret_cast<struct sockaddr *>(&sockaddr_listen),
        ...
        close(m_listen_fd);
        set_error_info(NET_IO_SERVER_EINIT, "fail to bind(%s)", strerror(errno));
        TRACE_LOG("ERROR: bind: %s", strerror(errno));
        return -1;
    }
    ...
    if (listen(m_listen_fd, NET_IO_SERVER_LISTEN_BACKLOG) != 0) {
        close(m_listen_fd);
        set_error_info(NET_IO_SERVER_EINIT, "fail to listen(%s)", strerror(errno));
        return -1;
    }
    ...
    if (epoll_ctl(m_epoll_fd, EPOLL_CTL_ADD, m_listen_fd, &event) != 0) {
        close(m_listen_fd);
        close(m_epoll_fd);
        set_error_info(NET_IO_SERVER_EINIT, "fail to EPOLL_CTL_ADD(%s)", strerror(errno));
        return -1;
    }
    ...
    if (p_net_io_notifier) {
        ...
        if (efd < 0) {
            close(m_listen_fd);
            close(m_epoll_fd);
            set_error_info(NET_IO_SERVER_EINIT, "fail to get_fd by net_io_notifier");
            return -1;
        }
        ...
        m_p_net_io_notifier = const_cast<i_net_io_notifier *>(p_net_io_notifier);
    }
...
int net_io_server::do_io(int timeout, int cmd)
{
    ...
    for (int i = 0; i < epoll_ret; ++ i) {
        ...
        if ((!m_events_[i].data.ptr) || (0 == i && (m_flag & NET_IO_SERVER_FLAG_RUNOUT))) {
            for (;;) {
                ...
                if (peer_socket >= 0) {
                    ...
                    std::pair<id_conn_map_iter_t, bool> ret = m_id_conn_map.insert(
                    ...
                    if (ret.second) {
                        ...
                        if (epoll_ctl(m_epoll_fd, EPOLL_CTL_ADD, peer_socket, &event) != 0) {
                            set_error_info(NET_IO_SERVER_EIO, "fail to EPOLL_CTL_ADD(%s)", strerror(errno));
                            close_connection(m_conn_info.id, true);
                            TRACE_LOG("ERROR: epoll_ctl: %s", strerror(errno));
                        }
                    } else {
                        // id冲突了,极低概率事件
                        set_error_info(NET_IO_SERVER_EIO, "fatal error:id conflict");
                        TRACE_LOG("ERROR: id conflict");
                    ...
                    if (EAGAIN == errno) {
                        ...
                        m_flag &= ~NET_IO_SERVER_FLAG_RUNOUT;
                        break;
                    } else if (EINTR == errno) {
                        // no thing to do ...
                    } else {
        ...
        if (m_p_net_io_notifier && m_events_[i].data.ptr == reinterpret_cast<void *>(-1)) {
            // 响应通知事件
            m_p_net_io_notifier->got();
            continue;
        }
        ...
        if (m_events_[i].data.ptr) {
            ...
            if ((m_events_[i].events & EPOLLERR) || (m_events_[i].events & EPOLLHUP) || 
                ...
                TRACE_LOG("ERROR: close_connection");
                ...
                if (rv > 0) {
                    if (p_conn_info->recv_buff_data_len > 0 && m_p_net_io_event_handler != NULL) {
                        ...
                        if (p_merge_addr == NULL) {
                            TRACE_LOG("ERROR: p_merge_addr == NULL");
                            close_connection(p_conn_info->id, true);
                        ...
                        if (left_data_len < 0) {
                            TRACE_LOG("ERROR: m_p_net_io_event_handler->on_recv_data: %d", left_data_len);
                            close_connection(p_conn_info->id, true);
                            m_events_[i].data.ptr = p_conn_info = NULL;
                            continue;
                        }
                    ...
                    TRACE_LOG("DEBUG: connection closed by peer party");
                    close_connection(p_conn_info->id, true);
                    m_events_[i].data.ptr = p_conn_info = NULL;
                } else {
                    TRACE_LOG("ERROR: rv: %d", rv);
                    close_connection(p_conn_info->id, true);
                    m_events_[i].data.ptr = p_conn_info = NULL;
                }
            } // if
...
int net_io_server::send_data(int id, const char *p_data, int data_len)
{
    ...
    if (p_data == NULL || data_len <= 0) {
        TRACE_LOG("ERROR: p_data == NULL || data_len <= 0");
        return -1;
    }

    id_conn_map_iter_t it = m_id_conn_map.find(id);
    if (it == m_id_conn_map.end()) {
        TRACE_LOG("ERROR: it == m_id_conn_map.end(): id: %d\n", id);
        set_error_info(NET_IO_SERVER_ESEND, "fail to find connection info according by id(%d)", id);
        return -1;
    }
...
{
    TRACE_LOG("id: %d, data_len: %d, is_atomic: %d", id, data_len, is_atomic);
    ...
    id_conn_map_iter_t iter;
    id_conn_map_iter_t temp = m_id_conn_map.begin();
    while (temp != m_id_conn_map.end()) {
        iter = temp; temp++;
        if (iter->first != id) {
            if (is_atomic && send_data(reinterpret_cast<conn_info_t *>(&iter->second), p_data, data_len) < 0) {
                ++ret;
            } else if (!is_atomic && send_data(reinterpret_cast<conn_info_t *>(&iter->second),
                            p_data,
                            data_len) != data_len) {
                ++ret;
            } else {
...
int net_io_server::close_connection(int id, bool need_notify)
{
    ...
    if (it == m_id_conn_map.end()) {
        TRACE_LOG("ERROR: fail to find connection info according by id: %d", id);
        set_error_info(NET_IO_SERVER_ECLOSE, "fail to find connection info according by id(%d)", id);
        return -1;
    }

    TRACE_LOG("%s:%d conn_id: %d conn_fd: %d", it->second.ip, it->second.port, it->second.id, it->second.fd);
                                         ...
                                         int *p_fd, union net_io_storage **pp_storage)
{
    ...
    if (it == m_id_conn_map.end()) {
        set_error_info(NET_IO_SERVER_ECLOSE, "fail to find connection info according by id(%d)", id);
        TRACE_LOG("ERROR: it == m_id_conn_map.end(): id: %d", id);
        return -1;
    }
...
int net_io_server::enum_connections(int *p_buffer, int length, int *p_buffer_len)
{
    ...
    for (id_conn_map_iter_t it = m_id_conn_map.begin(); (data_len < length) && (it != m_id_conn_map.end()); ++ it) {
...
{
    TRACE_LOG("net_io_server::uninit");
    if (!m_inited) {
        TRACE_LOG("ERROR: !m_inited");
        return -1;
    }
    ...
    while (temp != m_id_conn_map.end()) {
        iter = temp; temp++;
        close_connection(iter->second.id, true);
    }
    ...
    list_for_each_entry_safe (p_mem_block, p_mem_temp, &m_free_mem_head, list) {
        ...
        TRACE_LOG("free: p_mem_block: %p", p_mem_block);
        free(p_mem_block);
    }

    if (m_p_cont_mem != NULL) {
        TRACE_LOG("free: m_p_cont_mem: %p m_cont_mem_len: %d", m_p_cont_mem, m_cont_mem_len);
        free(m_p_cont_mem);
        m_p_cont_mem = NULL;
        m_cont_mem_len = 0;
    }

    m_errno = NET_IO_SERVER_OK;
    bzero(m_errstr, sizeof(m_errstr));
    m_flag = 0;
    m_inited = false;
...
void net_io_server::set_error_info(int error_no, const char *msg, ...)
{
    m_errno = error_no;
    va_list args;
    va_start(args, msg);
    vsprintf(m_errstr, msg, args);
    va_end(args);
}
...
int net_io_server::alloc_mem_block(struct list_head *head)
{
    if (list_empty(&m_free_mem_head)) {
        ...
        TRACE_LOG("malloc: size: %d p_mem_block: %p", (int)sizeof(mem_block_t) + MEM_BLOCK_SIZE, p_mem_block);
        if (p_mem_block == NULL) {
            TRACE_LOG("ERROR: malloc");
            return -1;
        }
    ...
    for (;;) {
        ...
        if (used_len % MEM_BLOCK_SIZE == 0) {
            if (alloc_mem_block(&p_conn_info->recv_buff_head) != 0) {
                TRACE_LOG("ERROR: alloc_mem_block");
                return -1;
            }
        }
        ...
        if (rv > 0) {
            ...
            TRACE_LOG("DEBUG: connection close by peer party");
            if (used_len % MEM_BLOCK_SIZE == 0) {
            ...
            TRACE_LOG("ERROR: recv: %s", strerror(errno));
            if (used_len % MEM_BLOCK_SIZE == 0) {
                free_mem_block(p_conn_info->recv_buff_head.prev);
            }
            return -1;
...
char * net_io_server::merge_mem_block(struct list_head *head)
{
    ...
    list_for_each_entry (p_mem_block, head, list) {
        if (idle_cont_len == 0) {
            ...
            TRACE_LOG("realloc: m_p_cont_mem: %p size: %d", m_p_cont_mem, m_cont_mem_len + MEM_BLOCK_SIZE);
            if (m_p_cont_mem == NULL) {
                TRACE_LOG("ERROR: m_p_cont_mem == NULL");
                return NULL;
            }
    ...
    if (p_conn_info->send_buff_data_len == 0) {
        while (left_data_len > 0) { 
            ...
            if (rv > 0) {
                ...
                TRACE_LOG("ERROR: send: %s", strerror(errno));
                return -1;
            }
        }
    }
