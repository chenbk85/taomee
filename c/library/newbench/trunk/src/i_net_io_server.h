...
struct i_net_io_event_handler
{
    ...
    * @return -1-关闭这个连接
    */
    virtual int on_new_connection(i_net_io_server *p_net_io_server, int id, int fd,
                                  const char *ip, int port, union net_io_storage *p_storage) = 0;
     ...
     */
    virtual int on_recv_data(i_net_io_server *p_net_io_server, int id, int fd,
     ...
     * @return
     */
    virtual int on_connection_closed(i_net_io_server *p_net_io_server,
                                     int id, int fd,
                                     const char *ip, int port, union net_io_storage *p_storage) = 0;
     ...
     * @return
     */
    virtual int on_wakeup(i_net_io_server *p_net_io_server) = 0;
};
...
struct i_net_io_server
{
     ...
     * @return
     */
    virtual int broadcast(int id, const char *p_data, int date_len, int is_atomic) = 0;
     ...
     * @return 0-成功 -1-失败
     */
    virtual int get_connection_info(int id, char *ip, int ip_buffer_len, int *p_port, int *p_time,
                                    int *p_fd, union net_io_storage **pp_storage) = 0;
     ...
     * @return 0-成功 -1-失败
     */
    virtual int enum_connections(int *p_buffer, int length, int *p_buffer_len) = 0;
