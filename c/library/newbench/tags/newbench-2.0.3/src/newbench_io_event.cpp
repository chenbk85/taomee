...
{
    DEBUG_LOG("%s:%d conn_id: %d conn_fd: %d", peer_ip, peer_port, connection_id, connection_fd);
                                          ...
                                          char *p_data, int data_len, union net_io_storage *p_storage)
{
    ...
    DEBUG_LOG("recv %dB data from %s:%d", data_len, ip, port);
...
//                p_net_io_server->close_connection(connection_id, true);
//                break;
                return -1;
            }
        }
        ...
        if (g_bench_conf.use_barrier) {
            ...
            if (ret != 0) {
                ...
                DEBUG_LOG("Barrier push data failed, discard pkg (len: %d): %s.", pkg_len, g_prcv_brq->get_last_errstr());
//                p_net_io_server->close_connection(connection_id, true);
//                break;
                return -1;
            } else {
            ...
            if (g_prcv_rq->push_data(m_push_buf, m_push_mb->length, true) != m_push_mb->length) {
                ...
                DEBUG_LOG("Common ring queue push failed(ret: %d, data: %d, empty: %d), discard pkg (len: %d)",
                            g_prcv_rq->get_last_errno(), g_prcv_rq->get_data_len(), g_prcv_rq->get_empty_buffer_len(), pkg_len);
//                p_net_io_server->close_connection(connection_id, true);
//                break;
                return -1;
...
int newbench_net_io_event::on_connection_closed(i_net_io_server * /* p_net_io_server */,
                                                  ...
                                                  union net_io_storage *p_storage)
{
    DEBUG_LOG("%s:%d conn_id: %d conn_fd: %d", ip, port, connection_id, connection_fd);
    ...
    while (!g_stop) {
        ...
        if (ret == 0) {
            ...
            if (ret != mb->length || mb->length > MAX_PACKAGE_LEN) {
                // 异常错误
                ERROR_LOG("exception: data length[%d] != package length or > MAX_PACKAGE_LEN", mb->length);
                break;
            }
        }
