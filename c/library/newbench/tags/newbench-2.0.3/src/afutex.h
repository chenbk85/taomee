                            ...
                            const struct timespec *p_tsp = NULL)
{
    switch (op_flag) {
        case AFUTEX_OP_WAIT:
            ...
            if (-1 == syscall(SYS_futex, &p_afutex_opbuf->mutex, FUTEX_WAIT, 1, p_tsp, NULL, 0)
                && ETIMEDOUT == errno) {
        ...
        case AFUTEX_OP_WAKE:
            if (p_afutex_opbuf->cnter > 0) {
                int ret = syscall(SYS_futex, &p_afutex_opbuf->mutex, FUTEX_WAKE, 1);
        ...
        case AFUTEX_OP_WAKE_ALL:
            if (p_afutex_opbuf->cnter > 0) {
                int ret = syscall(SYS_futex, &p_afutex_opbuf->mutex, FUTEX_WAKE, p_afutex_opbuf->cnter);
