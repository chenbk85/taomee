...
static i_ini_file *g_p_ini = NULL;
...
int config_init()
{
    ...
    ///初始化RunInfo
	rv = g_p_ini->read_string("RunInfo", "run_mode", g_bench_conf.run_mode, MAX_CONF_STR_LEN, "");
    if (rv != 0) {
        print_prompt(false, "Get run_mode config failed(%d)!", rv);
        return -1;
    }
