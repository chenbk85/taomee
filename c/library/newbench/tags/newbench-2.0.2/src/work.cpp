...
{
    DEBUG_LOG("connection_id: %d data_len: %d, is_broadcast: %d", connection_id, data_len, is_broadcast);
...
{
    DEBUG_LOG("connection_id: %d", connection_id);
    if (g_send_buff_len < (int)sizeof(shm_block_t)) {
        g_p_send_buff = (char *)realloc(g_p_send_buff, sizeof(shm_block_t));
        DEBUG_LOG("work_proc: realloc: %p %d", g_p_send_buff, (int)sizeof(shm_block_t));
        if (g_p_send_buff == NULL) {
            ERROR_LOG("g_p_send_buff == NULL");
            return -1;
        }
