                        ...
                        int id_base, int id_step)
{
    ...
    if (!ip || port < 0 || port > 65535) {
        ERROR_LOG("!ip || port < 0 || port > 65535");
        return -1;
    }

    if (id_base <= 0 || id_step <= 0) {
        ERROR_LOG("id_base <= 0 || id_step <= 0");
        return -1;
    }
    ...
    if (bind(m_listen_fd, reinterpret_cast<struct sockaddr *>(&sockaddr_listen),
        ...
        ERROR_LOG("bind: %s", strerror(errno));
        return -1;
    }
            ...
            if (m_events_[i].events & EPOLLIN) {
                ...
                if (rv > 0) {
                    if (p_conn_info->recv_buff_data_len > 0 && m_p_net_io_event_handler != NULL) {
                        ...
                        if (left_data_len < 0) {
                            DEBUG_LOG("m_p_net_io_event_handler->on_recv_data: %d", left_data_len);
                            ERROR_LOG("m_p_net_io_event_handler->on_recv_data: %d", left_data_len);
                            close_connection(p_conn_info->id, true);
                            m_events_[i].data.ptr = p_conn_info = NULL;
                            continue;
                        }
...
{
    DEBUG_LOG("id: %d, data_len: %d, is_atomic: %d", id, data_len, is_atomic);
...
int net_io_server::close_connection(int id, bool need_notify)
{
    ...
    if (it == m_id_conn_map.end()) {
        DEBUG_LOG("fail to find connection info according by id: %d", id);
        ERROR_LOG("fail to find connection info according by id: %d", id);
        set_error_info_(NET_IO_SERVER_ECLOSE, "fail to find connection info according by id(%d)", id);
        return -1;
    }

    DEBUG_LOG("%s:%d conn_id: %d conn_fd: %d", it->second.ip, it->second.port, it->second.id, it->second.fd);
...
int net_io_server::recv_data(conn_info_t *p_conn_info)
{
    ...
    for (;;) {
        ...
        if (rv > 0) {
            ...
            ERROR_LOG("recv: %s", strerror(errno));
            if (used_len % MEM_BLOCK_SIZE == 0) {
                free_mem_block(p_conn_info->recv_buff_head.prev);
            }
            return -1;
    ...
    if (p_conn_info->send_buff_data_len == 0) {
        while (left_data_len > 0) { 
            ...
            if (rv > 0) {
                ...
                ERROR_LOG("send: %s", strerror(errno));
                return -1;
            }
        }
    }
