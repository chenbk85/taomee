...
{
    DEBUG_LOG("recv_len: %d", recv_len);
    
    if (recv_len >= (int)sizeof(uint32_t)) {
        int msg_len = (int)*(uint32_t *)p_recv;
        DEBUG_LOG("recv_len: %d msg_len: %u", recv_len, msg_len);
        if (recv_len >= msg_len) {
            DEBUG_LOG("recv a whole %dB msg", msg_len);
            return msg_len;
        }
    }

    DEBUG_LOG("recv_len: %d: continue to recv", recv_len);
...
//    send_data(p_skinfo->connection_id, p_recv, recv_len, 1, 1);
//    send_data(p_skinfo->connection_id, p_recv, recv_len, 1, 0);

//    close_connection(p_skinfo->connection_id); 
