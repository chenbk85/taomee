...
net_io_server::net_io_server() : m_inited(false), m_id_(0), m_errno_(NET_IO_SERVER_OK),
                                 m_p_net_io_notifier_(NULL), m_flag(0), 
...
{
    DEBUG_LOG("net_io_server::init");
    if (m_inited) {
        ERROR_LOG("ERROR: m_inited");
        return -1;
    }

    if (!ip || port < 0 || port > 65535) {
        ERROR_LOG("ERROR: !ip || port < 0 || port > 65535");
        return -1;
    }

    if (id_base <= 0 || id_step <= 0) {
        ERROR_LOG("ERROR: id_base <= 0 || id_step <= 0");
        return -1;
    }
    ...
    if (bind(m_listen_fd, reinterpret_cast<struct sockaddr *>(&sockaddr_listen),
        ...
        ERROR_LOG("ERROR: bind: %s", strerror(errno));
        return -1;
    }
                ...
                if (rv > 0) {
                    if (p_conn_info->recv_buff_data_len > 0 && m_p_net_io_event_handler != NULL) {
                        ...
                        if (p_merge_addr == NULL) {
                            DEBUG_LOG("DEBUG: merge_mem_block");
                            close_connection(p_conn_info->id, true);
                        ...
                        if (left_data_len < 0) {
                            DEBUG_LOG("DEBUG: m_p_net_io_event_handler->on_recv_data: %d", left_data_len);
                            ERROR_LOG("DEBUG: m_p_net_io_event_handler->on_recv_data: %d", left_data_len);
                            close_connection(p_conn_info->id, true);
                            m_events_[i].data.ptr = p_conn_info = NULL;
                            continue;
                        }
                    ...
                    DEBUG_LOG("DEBUG: connection closed by peer party");
                    close_connection(p_conn_info->id, true);
                    m_events_[i].data.ptr = p_conn_info = NULL;
                } else {
                    ERROR_LOG("ERROR: recv_data");
                    close_connection(p_conn_info->id, true);
                    m_events_[i].data.ptr = p_conn_info = NULL;
                }
            } // if
...
int net_io_server::send_data(int id, const char *p_data, int data_len)
{
    ...
    if (p_data == NULL || data_len <= 0) {
        ERROR_LOG("p_data == NULL || data_len <= 0");
        return -1;
    }

    id_conn_map_iter_t it = m_id_conn_map.find(id);
    if (it == m_id_conn_map.end()) {
        ERROR_LOG("it == m_id_conn_map.end(): id: %d\n", id);
        DEBUG_LOG("it == m_id_conn_map.end(): id: %d\n", id);
...
int net_io_server::close_connection(int id, bool need_notify)
{
    ...
    if (it == m_id_conn_map.end()) {
        DEBUG_LOG("ERROR: fail to find connection info according by id: %d", id);
        ERROR_LOG("ERROR: fail to find connection info according by id: %d", id);
        set_error_info_(NET_IO_SERVER_ECLOSE, "fail to find connection info according by id(%d)", id);
        return -1;
    }

    DEBUG_LOG("close connection with: %s:%d", it->second.ip, it->second.port);
...
{
    DEBUG_LOG("net_io_server::uninit");
    if (!m_inited) {
        ERROR_LOG("ERROR: !m_inited");
        return -1;
    }
    ...
    list_for_each_entry_safe (p_mem_block, p_mem_temp, &m_free_mem_head, list) {
        ...
        DEBUG_LOG("free: p_mem_block: %p", p_mem_block);
        free(p_mem_block);
    }

    if (m_p_cont_mem != NULL) {
        DEBUG_LOG("free: m_p_cont_mem: %p m_cont_mem_len: %d", m_p_cont_mem, m_cont_mem_len);
        free(m_p_cont_mem);
        m_p_cont_mem = NULL;
        m_cont_mem_len = 0;
    }
...
int net_io_server::alloc_mem_block(struct list_head *head)
{
    if (list_empty(&m_free_mem_head)) {
        ...
        DEBUG_LOG("malloc: size: %d p_mem_block: %p", (int)sizeof(mem_block_t) + MEM_BLOCK_SIZE, p_mem_block);
        memset(p_mem_block, 0, sizeof(mem_block_t) + MEM_BLOCK_SIZE);
        if (p_mem_block == NULL) {
            ERROR_LOG("ERROR: malloc");
            return -1;
        }
    ...
    for (;;) {
        ...
        if (used_len % MEM_BLOCK_SIZE == 0) {
            if (alloc_mem_block(&p_conn_info->recv_buff_head) != 0) {
                ERROR_LOG("ERROR: alloc_mem_block");
                return -1;
            }
        }
        ...
        if (rv > 0) {
            ...
            DEBUG_LOG("DEBUG: connection close by peer party");
            if (used_len % MEM_BLOCK_SIZE == 0) {
            ...
            ERROR_LOG("ERROR: recv: %s", strerror(errno));
            if (used_len % MEM_BLOCK_SIZE == 0) {
                free_mem_block(p_conn_info->recv_buff_head.prev);
            }
            return -1;
...
char * net_io_server::merge_mem_block(struct list_head *head)
{
    ...
    list_for_each_entry (p_mem_block, head, list) {
        if (idle_cont_len == 0) {
            ...
            DEBUG_LOG("realloc: m_p_cont_mem: %p size: %d", m_p_cont_mem, m_cont_mem_len + MEM_BLOCK_SIZE);
            if (m_p_cont_mem == NULL) {
                ERROR_LOG("m_p_cont_mem == NULL");
                return NULL;
            }
    ...
    if (p_conn_info->send_buff_data_len == 0) {
        while (left_data_len > 0) { 
            ...
            if (rv > 0) {
                ...
                DEBUG_LOG("ERROR: send: %s", strerror(errno));
                ERROR_LOG("ERROR: send: %s", strerror(errno));
                return -1;
            }
        }
    }
