 ...
 *
 *   @internal
 *     Created  08/04/2010 10:33:53 AM
 *    Revision  1.0.0.0
 *    Compiler  gcc/g++
...
static char *g_p_buffer_send = NULL;
...
int spawn_work_proc(int argc, char *argv[], pid_t *wpids, int work_count, bool use_barrier)
{
    for (int i = 0; i < work_count; ++i)
    {
        ...
        if(wpids[i] < 0)
        {
            NB_BOOT_LOG(-1, "Fork work fail: %s", strerror(errno));
            return -1;
        }
        else if(wpids[i] == 0)
        {//子进程
    ...
    NB_BOOT_LOG(0, "Forked %d work processes.", work_count);
    return 0;
}
...
int send_data(int connection_id,
              const char *p_data,
              int data_len,
              int /* is_atomic */,
              int is_broadcast)
{
    shm_block_t *mb = (shm_block_t *)g_p_buffer_send;
...
int run_work_proc(int argc, char *argv[], bool /* use_barrier */)
{
    ...
    {
        NB_BOOT_LOG(-1, "Worker process handle_init failed.");
        return -1;
    }
    ...
    g_p_buffer_send = (char *)malloc(prcv_data_len);

    if (prcv_data == NULL)
    {
        NB_BOOT_LOG(-1, "Malloc work proc pop buffer fail, size: %d B", prcv_data_len);
        return -1;
    }
    ...
    if (g_bench_conf.use_barrier == true)
    {
        if (g_prcv_brq == NULL)
        {
            NB_BOOT_LOG(-1, "Use barrier mode, but i_barrier_ring_queue is null.");
            goto work_proc_end;
        }
    ...
    else
    {
        if (g_prcv_rq == NULL)
        {
            NB_BOOT_LOG(-1, "Use nonbarrier mode, but i_ring_queue object is null.");
            goto work_proc_end;
        }
        ...
        while (!g_stop)
        {
            ...
            ret = g_prcv_rq->pop_data_dummy(NULL, 4096 * 4096, 0);
