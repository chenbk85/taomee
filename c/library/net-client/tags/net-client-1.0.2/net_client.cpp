...
{
    int rv = -1;
    int rqst_msg_rmng_len = 0;
    int resp_buf_used_len = 0;

    if (byte_count == TWO_BYTE && byte_order == HOST_BYTE_ORDER) {
        rqst_msg_rmng_len = *(uint16_t *)rqst_msg;
    } else if (byte_count == TWO_BYTE && byte_order == NET_BYTE_ORDER) {
        rqst_msg_rmng_len = ntohs(*(uint16_t *)rqst_msg);
    } else if (byte_count == FOUR_BYTE && byte_order == HOST_BYTE_ORDER) {
        rqst_msg_rmng_len = *(uint32_t *)rqst_msg;
    } else if (byte_count == FOUR_BYTE && byte_order == NET_BYTE_ORDER) {
        rqst_msg_rmng_len = ntohl(*(uint32_t *)rqst_msg);
    } else {
        snprintf(m_error_str, sizeof(m_error_str), "parameter error");
        return -1;
    }
    ...
    for (;;) {
        ...
        if (FD_ISSET(m_sockfd, &read_set)) {
            ...
            if (rv == -1) {
                ...
                if (resp_buf_used_len >= byte_count) {
                    int resp_msg_len = 0;
                    
                    if (byte_count == TWO_BYTE && byte_order == HOST_BYTE_ORDER) {
                        resp_msg_len = *(uint16_t *)resp_buf;
                    } else if (byte_count == TWO_BYTE && byte_order == NET_BYTE_ORDER) {
                        resp_msg_len = ntohs(*(uint16_t *)resp_buf);
                    } else if (byte_count == FOUR_BYTE && byte_order == HOST_BYTE_ORDER) {
                        resp_msg_len = *(uint32_t *)resp_buf;
                    } else if (byte_count == FOUR_BYTE && byte_order == NET_BYTE_ORDER) {
                        resp_msg_len = ntohl(*(uint32_t *)resp_buf);
                    } else {
                        snprintf(m_error_str, sizeof(m_error_str), "parameter error");
                        return -1;
                    }

                    if (resp_buf_used_len >= resp_msg_len) {
                        break;    // receive the whole message, so break the for loop.
                    }
                }
            }
        }
