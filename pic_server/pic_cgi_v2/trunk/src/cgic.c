...
int main(int argc, char *argv[]) {
	...
	if (strchr(cgiContentType, ';')) {
		...
		while (sat) {
			*sat = '\0';
			...
			if (cgiStrBeginsNc(sat, (char*)"boundary=")) {
				char *s;
	...
	if (cgiStrEqNc(cgiRequestMethod,(char*)"post")) {
#ifdef CGICDEBUG
		CGICDEBUGSTART
		fprintf(dout, "POST recognized\n");
		CGICDEBUGEND
#endif /* CGICDEBUG */
		if (cgiStrEqNc(cgiContentType,(char*)"application/x-www-form-urlencoded")) {	
#ifdef CGICDEBUG
			CGICDEBUGSTART
			...
			CGICDEBUGEND	
#endif /* CGICDEBUG */
		} else if (cgiStrEqNc(cgiContentType,(char*)"multipart/form-data")) {
#ifdef CGICDEBUG
			CGICDEBUGSTART
	...
	} else if (cgiStrEqNc(cgiRequestMethod,(char*)"get")) {	
...
static cgiParseResultType cgiParsePostMultipartInput() {
	...
	while (1) {
				...
				mpp, attr, sizeof(attr), value, sizeof(value))) 
		{
			...
			if (cgiStrEqNc(attr, (char*)"Content-Disposition")) {
				...
				decomposeValue(value, 
					...
					argValues,
					1024);	
			} else if (cgiStrEqNc(attr, (char*)"Content-Type")) {
				argNames[0] = 0;
				decomposeValue(value, 
		...
		if (!cgiStrEqNc(fvalue, (char*)"form-data")) {
			/* Not form data */	
			continue;
		}
...
cgiFormResultType cgiGetFilePath(char *name,char *result,int resultSpace){
	cgiFormEntry *e;
	char *s;
	int resultLen = 0;
...
cgiEnvironmentResultType cgiWriteEnvironment(char *filename) {
	...
	if (!cgiWriteString(out, (char*)"CGIC2.0")) {
		goto error;
	}
