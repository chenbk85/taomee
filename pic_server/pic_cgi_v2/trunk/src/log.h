...
extern void write_log(int lvl,uint32_t key, const char* fmt, ...);
extern void write_syslog(int lvl, const char* fmt, ...);
...
/**
 * @def CGI_ERROR_LOG
 * @brief 输出log_lvl_error等级日志。如果定义宏DISABLE_CGI_ERROR_LOG，则可以在编译期把CGI_ERROR_LOG去除。\n
 *        用法示例：CGI_ERROR_LOG("dlopen error, %s", error);
 */
/**
 * @def KCGI_ERROR_LOG
 * @brief 输出log_lvl_error等级日志。比CGI_ERROR_LOG多一个参数 key,一般写入userid。 如果定义宏DISABLE_CGI_ERROR_LOG，则可以在编译期把CGI_ERROR_LOG去除。\n
 *        用法示例：KCGI_ERROR_LOG(userid, "dlopen error, %s", error);
 */

#ifndef DISABLE_CGI_ERROR_LOG
#define CGI_ERROR_LOG(fmt, args...) \
		DETAIL(log_lvl_error, 0, fmt, ##args)
#define KCGI_ERROR_LOG(key, fmt, args...) \
...
/**
 * @def KINFO_LOG
 * @brief 输出log_lvl_info等级日志。比CGI_ERROR_LOG多一个参数 key,一般写入userid。如果定义宏DISABLE_INFO_LOG，则可以在编译期把KINFO_LOG去除。\n
...
/**
 * @def CGI_DEBUG_LOG
 * @brief 输出log_lvl_debug等级日志。如果定义宏DISABLE_CGI_DEBUG_LOG，则可以在编译期把CGI_DEBUG_LOG去除。\n
 *        用法示例：CGI_DEBUG_LOG("dlopen error, %s", error);
 */
/**
 * @def KCGI_DEBUG_LOG
 * @brief 输出log_lvl_debug等级日志。比CGI_DEBUG_LOG 多一个参数 key,一般写入userid。 如果定义宏DISABLE_CGI_DEBUG_LOG，则可以在编译期把KCGI_DEBUG_LOG去除。\n
 *        用法示例：KCGI_DEBUG_LOG(userid, "dlopen error, %s", error);
 */

#ifndef DISABLE_CGI_DEBUG_LOG
#define CGI_DEBUG_LOG(fmt, args...) \
		SIMPLY(log_lvl_debug, 0, fmt, ##args)
#define KCGI_DEBUG_LOG(key, fmt, args...) \
 ...
 *        如果编译时定义了宏LOG_USE_SYSLOG，则TRACE_LOG日志会写到CGI_DEBUG_LOG日志文件里。\n
 ...
 *        如果编译时定义了宏LOG_USE_SYSLOG，则KTRACE_LOG日志会写到KCGI_DEBUG_LOG日志文件里。\n
