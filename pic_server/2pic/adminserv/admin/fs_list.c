...
{
	if (g_webproxy_fd == -1) {
		g_webproxy_fd = connect_to_svr(config_get_strval("webproxy_ip"), WEB_PROXY_PORT, 8*1024, 1);
		if (g_webproxy_fd == -1) {
			ERROR_RETURN(("cannot connect to webserv\t"), -1);
		}
	}
	return net_send(g_webproxy_fd, buf, len);
}

int send_to_webserv(void* buf, int len)
{
	if (g_websvr_fd == -1) {
		g_websvr_fd = connect_to_svr(config_get_strval("webserv_ip"), 80, 8*1024, 1);
		if (g_websvr_fd == -1) {
			ERROR_RETURN(("cannot connect to webserv\t"), -1);
		}
	}
	return net_send(g_websvr_fd, buf, len);
}
