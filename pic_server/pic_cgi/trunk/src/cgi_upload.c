...
recv_fileserver_t* received_from_fileserver(char* pkg,int pkglen)
{
	...
	CGI_DEBUG_LOG("RECV HOSTID[%u] lloccode[%s] strlen[%u] filelen[%u]",
					pkg_recv->hostid,
					pkg_recv->lloccode,
...
recv_fileserver_t* connect_with_fileserver(const char* buffer,Ctcp* conn_fileserver,recv_webclient_t *web_pkg, int filelen,uint32_t pic_type,uint32_t branchtype,const char* path)
{
	...
	CGI_DEBUG_LOG("SEND [ %u %u %u %u %u %s ]",
							web_pkg->channel,
							pic_type,
							branchtype,
							web_pkg->width_limit,
...
int parse_web_data(const char* s_data,recv_webclient_t *d_data)
{
	...
	CGI_DEBUG_LOG("RECV WEB[%u %u %u %u %u %u]",d_data->userid,d_data->channel,d_data->ip,d_data->time,d_data->width_limit,d_data->height_limit);
	return SUCC;
}
...
{	
	...
	json_object_object_add(my_object,"filelen",json_object_new_int(recv_data->len));
PRINT_END:
