...
int dispatch(int fd, const char * buf, uint32_t len)
{
    ...
    const cmd_proto_t * p_cmd = find_node_cmd(cmd);
    if (NULL == p_cmd)
    {
    ...
    if (NODE_SUCC == cmd_ret)
    {
        node_proto_t head;
        init_node_proto_header(&head, sizeof(head), seq, cmd, cmd_ret, uid);
        net_send_cli_msg(fd, &head, p_cmd->p_out);
    ...
    else
    {
        node_proto_t head;
        init_node_proto_header(&head, sizeof(head), seq, cmd, cmd_ret, uid);
        net_send_cli(fd, &head, sizeof(head));
    }
