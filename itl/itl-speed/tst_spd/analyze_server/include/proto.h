...
enum _proto_set_cdn_speed {
	proto_set_cdn_max_speed						= 0x6001,
	proto_set_cdn_speed_dis						= 0x6002,
};
...
typedef struct p_set_cdn_speed{
	uint32_t max_speed;
}__attribute__((packed)) proto_set_cdn_max_speed_t;

typedef struct p_set_cdn_speed_dis{
	uint32_t dis[4];	//A B C D======[0-A) [A-B) [B-C) [C-D) [D->MAX
}__attribute__((packed)) proto_set_cdn_speed_dis_t;
