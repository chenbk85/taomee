...
static int send_idc_ip_2_dbserver()
{
	...
	for (i = 0; i < idc_test_ip_mmap.records_num; i++) {
		...
		if ((idc_ip + i)->count == 0 && (idc_ip + i)->ip != 0
            ...
            && (i+1) < idc_test_ip_mmap.records_num)	{
			...
			//修改缓存对应的内容
			int k;
			idc_test_ip_avg_t *ptr;
			for (k = 0; k < max_idc_number; k++) {
				ptr = cache_idc_ip + k * MAX_IP_RECORD + i;
				ptr->isp_key = 0;
			}
		}
		//end-added by singku for remove ip which is out of date 2011-08-04
		int sum;
		if ((sum = (idc_ip + i)->count) == 0)
			continue;
		...
		if ((idc_ip + i)->count_link != 0 && (idc_ip + i)->idc_master != IDC_MASTER_INVALID) {
			...
			for (j = 0; j < MAX_LINK; j++) {
				...
				int sum_link = (idc_ip+i)->link_result[j].count;
				if (sum_link == 0)
					continue;
				if ((idc_ip+i)->link_result[j].delay/sum_link>1000
					||(idc_ip+i)->link_result[j].loss/sum_link>100
					||(idc_ip+i)->link_result[j].hop/sum_link>255) {
			...
			/*start---added by singku for cache 11-08-04*/
			if (idc_start == 1)	//程序刚运行时，全局的start = 0 处理旧的mmap不更新缓存;
										//开始processdir的时候start 变为1,以后的数据需要更新缓存
				update_cache(proto_cache_idc_ip, (void*)(idc_ip + i));
			/*end ----added by singku for cache 11-08-04*/
...
static int send_idc_area_2_dbserver()
{
	...
	for (i = 0; i < idc_test_area_mmap.records_num; i++) {
		int sum;
		if ((sum=(idc_area + i)->count) == 0)
			continue;

		/*start---added by singku for cache 11-07-27*/
		if (idc_start == 1)	//程序刚运行时，全局的start = 0 处理旧的mmap不更新缓存;
							//开始processdir的时候start 变为1,以后的数据需要更新缓存
			update_cache(proto_cache_idc_area, (void*)(idc_area + i));
