 ...
 */
int write_file(const char *filename, const char *buf, uint32_t data_len)
{
	FILE *resultfile;
	//char tmp_file_full_path_name[FULL_PATH_LEN];//临时文件全名
	char file_full_path_name[FULL_PATH_LEN];//文件全名,临时文件写完毕后改成这个名字

	if (my_mkdir( (const char *)data_store_path ) == -1)//递归创建目录
		return -1;

	//sprintf(tmp_file_full_path_name, "%s/tmp.%s", data_store_path, filename);

	sprintf(file_full_path_name,"%s/%s",data_store_path, filename);//以路径和时间命名

	resultfile = fopen(file_full_path_name, "ab+");	//以二进制方式写
	if (resultfile == NULL) {
	...
	//写文件
	if ( data_len != 0 && fwrite(buf, data_len, 1, resultfile) != 1)
		return -1;
