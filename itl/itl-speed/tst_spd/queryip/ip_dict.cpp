...
bool CIpDict::init(const char * file_path)
{
	uint32_t size = 0;
...
bool CIpDict::find(uint32_t ip, code_t &code, uint32_t *key)
{
	...
	if (hip == it->first) {
		if (it->second.province_code == 0)
			goto retf;
        //if (it->second.province_code == 830000) //国外的IP
        //    goto retf;
	...
	if (it->second.province_code == 0)
		goto retf;
    //if (it->second.province_code == 830000) //国外IP
    //    goto retf;
...
retf:
	printf("IP NOT FOUND :%u [possibly an ip used for mobile hot pot]\n",hip);
	return false;
}
