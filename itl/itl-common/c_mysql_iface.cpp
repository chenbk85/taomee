...
@return 成功返回0， 失败返回－1
*/
int c_mysql_iface::init(const char * sock, const char* db_name, const char* user, const char* passwd, const char* charset)
{
    if (m_hdb != NULL) {
        uninit();
    }
    ...
    if (mysql_real_connect(m_hdb, NULL, user, passwd, db_name, 0, sock, CLIENT_INTERACTIVE) == NULL) {
        set_err(MYSQL_IFACE_E_MYSQL_API, "Mysql_real_connect to %s on %s with user %s(%s) error: %s.", sock, db_name, user, passwd, mysql_error(m_hdb));
        return -1;
    }
...
int c_mysql_iface::init(const char* host, unsigned short port, const char* db_name, const char* user, const char* passwd, const char* charset)
{
    ...
    // // 强制使用tcp连接，防止host = localhost时，mysql_real_connect自动用socket连接到的实例与port不对应
    // unsigned int protocol_type = MYSQL_PROTOCOL_TCP;
    // if (mysql_options(m_hdb, MYSQL_OPT_PROTOCOL, (const char *)&protocol_type))
    // {
        // set_err(MYSQL_IFACE_E_MYSQL_API, "set mysql protocol type error: %s", mysql_error(m_hdb));
        // return -1;
    // }
...
int c_mysql_iface::execsql(const char* sql_fmt, ...)
{
    ...
    va_list argptr;
    va_start(argptr, sql_fmt);
    vsnprintf(m_sql_buf, sizeof(m_sql_buf), sql_fmt, argptr);
    va_end(argptr);
