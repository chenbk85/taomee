...
void c_mysql_iface::set_err(int errno, const char* msg, ...)
{
    ...
    va_list argptr;
    va_start(argptr, msg);
    vsnprintf(m_errstr, 1024, msg, argptr);
    va_end(argptr);
}
 ...
 */
int create_mysql_iface_instance(c_mysql_iface** pp_instance)
{
    if (pp_instance == NULL) {
        return -1;
    }
    ...
    *pp_instance = dynamic_cast<c_mysql_iface*>(p_instance);
    return 0;
}
