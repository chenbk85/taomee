...
{
    if (dbpkg->cmd == dbproto_safe_trade_item) {
        handle_safe_trade_db_dispatch(dbpkg, pkglen);
        return ;
    }
...
bool init_db_proto_handles()
{
	...
	SET_DB_HANDLE(buy_goods, sizeof(db_buy_goods_rsp_t), cmp_must_eq);
	SET_DB_HANDLE(sell_goods, sizeof(db_sell_goods_rsp_t), cmp_must_eq);
