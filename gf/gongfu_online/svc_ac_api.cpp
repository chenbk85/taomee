 ...
 */
void send_cpkg_head_to_svr_ex(uint32_t len, uint8_t ver, uint32_t cmd, uint32_t uid, uint32_t ret,
		const struct timeval *tv_now)
{
	struct sockaddr_in cliaddr;

	cliaddr.sin_family = AF_INET;
	inet_pton(AF_INET, branch_sys_ip, &(cliaddr.sin_addr));
	cliaddr.sin_port = htons(branch_sys_port);
	...
	chead.id = uid;
	chead.ret = ret;
	//printf("%s : %u\n", branch_sys_ip, branch_sys_port);
	send_cpkg_head_to_svr(branch_sys_ip, branch_sys_port,
			&chead, sizeof(chead), &cliaddr, tv_now);
}
...
//	size_t len = 0;
	...
	if (ac_unlikely(sizeof(struct __ac_one_cdata_t) > (uint32_t)(max_left))) {
		need_send = 1;
		goto try_send;
	}
...
try_send:
	if (need_send && tot_rcd_count) {
		...
		/* sendto */
		sendto(ac_udp_fd, svr_pkg_buf, tot_len, 0,
