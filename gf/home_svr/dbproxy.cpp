...
int send_request_to_db(Player* p, userid_t id, uint32_t role_tm, uint16_t cmd, const void* body_buf, uint32_t body_len)
{
	...
	if (p) {
		KDEBUG_LOG(p->id, "TO DB\t[uid=%u %u cmd=%u 0x%X]", p->id, id, p->waitcmd, cmd);
	}
...
void handle_db_return(db_proto_t* dbpkg, uint32_t pkglen)
{
	...
	Player* p = get_player(dbpkg->seq);
	if ((p == 0) || (p->id == 0) || (p->id != dbpkg->seq)) {
		ERROR_LOG("player leave already: uid=%u grpid=%u dbcmd=0x%X",
...
bool init_db_proto_handles()
{
	...
	SET_DB_HANDLE(get_plants_list, 4, cmp_must_ge);
	SET_DB_HANDLE(pick_fruit, 8, cmp_must_ge);
	SET_DB_HANDLE(get_summon_list, 4, cmp_must_ge);
	SET_DB_HANDLE(get_home_data, sizeof(home_attr_data_rsp_t), cmp_must_eq);	
//	SET_DB_HANDLE(replace_home_exp_level, sizeof(home_replace_exp_level_rsp_t), cmp_must_eq);

	SET_DB_HANDLE(get_decorate_list, 4, cmp_must_ge);
	SET_DB_HANDLE(set_decorate, 12, cmp_must_eq);
	return true;
}
