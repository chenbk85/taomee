...
int time_stamp_update(char *recbuf, int rcvlen, char *sendbuf, int *sndlen)
{
	...
	*sndlen = 0;
	...
	if (*(uint32_t *)recbuf != PROTO_HEAD_SIZE ) {
		//ERROR_LOG("the length is wrong, %u", ntohl(*(uint32_t *)recbuf));
		ERROR_LOG("the length is wrong, %u", *(uint32_t *)recbuf);
		return REG_SIZE_ERR;
	}
	...
	user_id = proto_data->user_id;
	DEBUG_LOG("REV USRID:%u",user_id);
	if (user_id >= max_user) {
	...
	time_mem[user_id].nick_time = time(0);         //将存储内容该为主机字节序
	DEBUG_LOG("user:%u,updatetime:%u",user_id,time_mem[user_id].nick_time);
	return 0;
}
...
int time_stamp_obtain(char *recbuf, int rcvlen, char *sendbuf, int *sndlen)
{
	...
	*(uint32_t *)(sendbuf + PROTO_HEAD_SIZE) = count;   //
	...
	for (i = 0; i < count; ++i, ++p_user_id, ++time_info) {
		...
		DEBUG_LOG("userid:%u,time:%u\n",time_info->user_id,time_info->time);
	}
