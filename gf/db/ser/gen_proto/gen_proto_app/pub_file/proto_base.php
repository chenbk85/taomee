<?php
function dbg_log($desc1,$desc2="",$desc3="",$desc4=""){
	$desc=var_export($desc1,true);
	$desc.="|".var_export($desc2,true);
	$desc.="|".var_export($desc3,true);
	$desc.="|".var_export($desc4,true);
    $date=strftime("%Y%m%d",time());
    $logfile="/tmp/proto.txt_$date";
    $fp = fopen($logfile, "a");
    flock($fp, LOCK_EX) ;
    fwrite($fp,strftime("%Y/%m/%d %H:%M:%S :",time()).$desc."\n");
    flock($fp, LOCK_UN);
    fclose($fp);
}
...
class Cbyte_array {
	...
	private $m_is_buf_hex_fmt;
	private $m_is_buf_url_fmt;
	private $m_buf;
	...
	public  function set_is_buf_hex_fmt($value){ $this->m_is_buf_hex_fmt=$value; }	
	public  function set_is_buf_url_fmt($value){ $this->m_is_buf_url_fmt=$value; }	
...
class Cproto_base{
	...
	function __construct( $payserip,$payserport){
		$this->sock = new proto_socket ($payserip,$payserport);
 		$this->is_buf_hex_fmt=false;
 		$this->is_buf_url_fmt=false;
	}
	function __destruct(){
	...
	function set_is_buf_hex_fmt($value){
		$this->is_buf_hex_fmt=$value;
	}
	function set_is_buf_url_fmt($value){
		$this->is_buf_url_fmt=$value;
	}
	...
	function send_cmd_new($cmdid ,$userid, $in_obj,$out_obj,$md5_tag=0 ){
		$ba=new Cbyte_array();
		$ba->set_is_buf_hex_fmt($this->is_buf_hex_fmt);
		$ba->set_is_buf_url_fmt($this->is_buf_url_fmt);
		$ba->init_write_mode();
		if ($in_obj){
		...
		unset ($ret_arr["pri_msg"]);

		#合并数组返回
		return $ret_arr;
	}

	function send_collect_cmd($collect_cmd ,$collect_seq,$cmd_arr )
	{
		$ba=new Cbyte_array();
		$ba->set_is_buf_hex_fmt($this->is_buf_hex_fmt);
		$ba->write_uint32($collect_seq);
		$ba->write_int32(0);
		$ba->write_uint16(0);
		$ba->write_uint32(count($cmd_arr));
		foreach ( $cmd_arr as $item ){
			$cmdid=$item["cmdid"];
			$msg_obj=$item["msg"];
			$ba->write_uint16($cmdid);
			if($msg_obj!=NULL){
				$msg_obj->write_to_buf($ba);
			}
		}

		$pri_msg=$ba->get_buf();
		$sendbuf=$this->pack( $cmdid , $userid,  $pri_msg);
		return $this->unpack($this->sock->sendmsg($sendbuf),"s*out_msg");
	}
