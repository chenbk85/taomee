...
//加入
Cgf_waction::Cgf_waction(mysql_interface * db ) 
	:Ctable( db, "GF_OTHER", "t_gf_weekly_action")
{ 

}

int Cgf_waction::get_action_type_list(userid_t userid,uint32_t role_regtime, uint32_t* p_count, 
...
int Cgf_waction::get_action_count(uint32_t userid, uint32_t role_regtime,
    uint32_t type, uint32_t *count)
{
	...
	STD_QUERY_ONE_BEGIN(this->sqlstr, GF_WEEK_ACTION_TYPE_NOFIND_ERR);
		INT_CPY_NEXT_FIELD( *count );
	STD_QUERY_ONE_END();
}

int Cgf_waction::insert_action(uint32_t userid, uint32_t role_regtime, uint32_t type)
{
    ...
    return this->exec_insert_sql (this->sqlstr, GF_WEEK_ACTION_TYPE_EXISTED_ERR );
}

int Cgf_waction::increase_action_count(uint32_t userid, uint32_t role_regtime,
    uint32_t type, uint32_t limit)
{
    uint32_t count = 0;
    int ret = this->get_action_count(userid, role_regtime, type, &count);
    if (ret != SUCC && ret != GF_WEEK_ACTION_TYPE_NOFIND_ERR){
        return  ret;
    } else if ( ret == GF_WEEK_ACTION_TYPE_NOFIND_ERR) {
        return this->insert_action(userid, role_regtime, type);
    }else {
        if (limit != 0 && count >= limit) {
            return GF_WEEK_ACTION_TYPE_MAX_ERR;
        }
        GEN_SQLSTR(this->sqlstr,"update %s set %s=%s+%u \
			where userid=%u and role_regtime=%u and typeid=%u; ",
			this->get_table_name(), "count", "count", 1, userid, role_regtime, type);
		return this->exec_update_sql (this->sqlstr, GF_WEEK_ACTION_TYPE_NOFIND_ERR);
    }
}
